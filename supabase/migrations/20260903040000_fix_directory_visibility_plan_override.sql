...
-- An explicit non-free plan_override is an administrative entitlement and must
-- behave like an active paid plan, matching the rest of the application.
...
begin
  ...
  -- plan_override in (pro, enterprise) is an administrative entitlement and is
  -- treated as active paid access regardless of Stripe/local status.
  select case
  ...
  order by case when s.plan_override in ('pro', 'enterprise') then 0
