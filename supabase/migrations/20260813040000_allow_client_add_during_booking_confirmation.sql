-- A client can be added from the appointment confirmation drawer before payment is finalized.
-- The appointment must belong to the authenticated user's barbershop and be either
-- scheduled or completed. The operation remains idempotent and tenant-scoped.
...
begin
  ...
  if v_user.id is null
     or v_user.role not in ('owner','admin','manager','barber','receptionist','staff') then
...
comment on function public.add_client_from_completed_appointment(uuid, uuid) is
  'Adds or links the appointment customer to the barbershop CRM after the appointment is confirmed or completed. Idempotent and tenant-scoped.';
