...
-- New invites use BARB-XXXX-XXXX, but short-lived legacy 10-character
-- hexadecimal invites must remain usable until their existing expiry window
-- closes. This avoids invalidating an already-issued invitation.
...
begin
  ...
  if v_input ~ '^BARB-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{4}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{4}$' then
    null;
  elsif v_input ~ '^[0-9A-F]{10}$' then
    -- Legacy format. Existing records are already hashed, so this is only
    -- accepted for unexpired/unused records created before the new format.
    null;
  else
