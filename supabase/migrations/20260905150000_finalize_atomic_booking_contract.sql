...
-- Final booking contract for the current migration chain.
-- This migration intentionally supersedes earlier recreate-functions so the
-- final database state contains one enum-safe, tenant-aware implementation.
...
declare
  ...
  v_status public.appointment_status := case
    when lower(trim(coalesce(p_status, 'scheduled'))) = 'pending'
...
begin
  ...
  select
    coalesce(b.closed_days, ''),
    coalesce(b.auto_confirm_bookings, false)
  into v_closed_days, v_auto_confirm
  ...
  v_local_dow := extract(dow from (p_date_hour at time zone 'Europe/Lisbon'))::integer;

  if exists (
    select 1
    from unnest(string_to_array(lower(v_closed_days), ',')) as day_value(value)
    where trim(day_value.value) = v_local_dow::text
      or trim(day_value.value) = case v_local_dow
        when 0 then 'sunday'
        when 1 then 'monday'
        when 2 then 'tuesday'
...
comment on function public.create_booking_atomic(uuid, uuid, uuid, timestamptz, integer, text, text, text, date, text) is
  'Final atomic public booking contract with enum-safe status, tenant-scoped conflict protection, closed-day enforcement and exact email+normalized-phone client linking.';
