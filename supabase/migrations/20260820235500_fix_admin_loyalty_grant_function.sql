...
-- The original admin function exposed an OUT column named `email`, which can
-- collide with the loyalty_members.email column inside PL/pgSQL. Drop the
-- previous signature so the return type can be safely renamed.
drop function if exists public.admin_grant_loyalty_points(uuid, text, integer, text);
...
begin
  ...
  if v_normalized_email = '' or length(v_normalized_email) > 254 then
  ...
  where lm.barbershop_id = p_barbershop_id
    and lower(lm.email) = v_normalized_email
