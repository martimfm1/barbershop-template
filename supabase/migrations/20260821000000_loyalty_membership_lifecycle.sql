...
begin
  ...
  values (
    ...
    v_email,
    nullif(trim(p_name), ''),
    greatest(v_settings.welcome_points, 0),
    'active',
    now(),
    now()
  )
...
create or replace function public.leave_loyalty_program(
  p_barbershop_id uuid,
  p_email text
)
returns boolean
...
create or replace function public.admin_find_loyalty_member_by_email(
  p_email text
)
returns table (
  member_id uuid,
...
create or replace function public.admin_grant_loyalty_points_by_email(
  p_email text,
  p_points integer,
...
begin
  ...
  if p_points is null or p_points <= 0 or p_points > 1000000 then
    raise exception 'LOYALTY_POINTS_INVALID';
  end if;
  ...
  where b.id = v_member.barbershop_id;

  if not found then
    raise exception 'LOYALTY_BARBERSHOP_NOT_FOUND';
  end if;
...
create unique index loyalty_members_one_active_email_idx
