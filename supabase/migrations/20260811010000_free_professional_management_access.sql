-- Free plan professional management access.
--
-- Product rule:
--   Free -> /equipa is available and the shop can create exactly 1 professional.
--   Free -> that professional always has 100% commission.
--
-- This migration fixes the authorization boundary of the atomic creation RPC.
-- Access to the feature is NOT a paid-plan entitlement; the quota is the
-- restriction. Existing tenant isolation and staff permission checks remain
-- in place.
...
begin
  ...
  -- Managing the team is available on every plan. Authorization is based on
  -- account role/explicit permission, not on the subscription plan.
  -- The barbershop creator (barbershop id == user id) is also treated as the
  -- owner so legacy accounts created before the role model remain functional.
  if v_role in ('admin', 'owner') or p_actor_user_id = p_barbershop_id then
    ...
    select exists (
      ...
      where sp.barbershop_id = p_barbershop_id
        ...
        and sp.permission in ('manage_professionals', 'team_management')
        and sp.allowed = true
  ...
  -- Free has team management, but no configurable commissions.
  v_commission := case
