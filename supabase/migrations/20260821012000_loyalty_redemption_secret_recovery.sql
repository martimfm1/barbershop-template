...
begin
  ...
  where lm.id = p_member_id and lm.status = 'active'
  for update;
...
revoke all on function public.redeem_loyalty_reward(uuid,uuid,text,text,text,text,timestamptz) from public, anon, authenticated;
grant execute on function public.redeem_loyalty_reward(uuid,uuid,text,text,text,text,timestamptz) to service_role;
