...
-- Final corrective migration for public booking creation.
--
-- Previous migrations repeatedly recreated this function and the latest
-- versions accidentally changed v_status back to text even though
-- appointments.status is public.appointment_status. PostgreSQL then raises
-- SQLSTATE 42804 during the INSERT. Keep the public API parameter as text,
-- but normalize it into the enum before writing the row.
...
begin
  ...
  -- Read the configured schedule from the tenant source of truth. The
  -- booking timestamp is interpreted in the barbershop's public timezone.
  select
    b.closed_days,
    coalesce(b.auto_confirm_bookings, false)
  ...
  v_local_date := (p_date_hour at time zone 'Europe/Lisbon')::date;
...
comment on function public.create_booking_atomic(uuid, uuid, uuid, timestamptz, integer, text, text, text, date, text) is
  'Atomic public booking creation with enum-safe status handling, tenant-scoped conflict protection, closed-day enforcement, and exact email+normalized-phone client linking.';
