...
-- Serialize appointment creation per barbershop and perform the final conflict
-- check inside the same transaction. This closes the race between API
-- availability checks and INSERTs, including "any professional" bookings.
...
begin
  ...
  -- A single short-lived lock per tenant serializes final booking decisions.
...
comment on function public.create_booking_atomic(uuid, uuid, uuid, timestamptz, integer, text, text, text, date, text) is
  'Atomic server-side appointment creation with tenant-scoped advisory locking and overlap protection.';
