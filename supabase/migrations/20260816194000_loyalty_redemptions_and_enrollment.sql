...
begin
  ...
  if not found then return new; end if;
  insert into public.loyalty_member_transactions (barbershop_id, member_id, points, type, reference_id, description)
  values (new.barbershop_id, v_member.id, v_rule.points, 'booking', new.id, v_rule.name)
  on conflict (member_id, reference_id) where type = 'booking' do nothing returning id into v_inserted;
  if v_inserted is not null then
    update public.loyalty_members set points_balance = points_balance + v_rule.points, updated_at = now() where id = v_member.id;
    begin
      update public.loyalty_accounts set points_balance = points_balance + v_rule.points, lifetime_points = lifetime_points + v_rule.points, updated_at = now()
      where barbershop_id = new.barbershop_id and client_id = (select id from public.users where lower(email) = lower(trim(new.manual_email)) limit 1);
...
create trigger loyalty_award_points_on_appointment after update of status on public.appointments for each row execute function public.award_loyalty_points_for_appointment();
