...
create index if not exists loyalty_members_barbershop_status_idx
  on public.loyalty_members (barbershop_id, status);
...
create index if not exists loyalty_earning_rules_shop_idx
  on public.loyalty_earning_rules (barbershop_id, active, created_at desc);
...
create index if not exists loyalty_member_transactions_member_idx
  on public.loyalty_member_transactions (member_id, created_at desc);
...
create unique index if not exists loyalty_redemptions_token_hash_idx
  on public.loyalty_redemptions (token_hash) where token_hash is not null;
create unique index if not exists loyalty_redemptions_code_hash_idx
  on public.loyalty_redemptions (code_hash) where code_hash is not null;
create index if not exists loyalty_redemptions_member_idx
  on public.loyalty_redemptions (member_id, created_at desc);
create index if not exists loyalty_redemptions_shop_pending_idx
  on public.loyalty_redemptions (barbershop_id, status, expires_at);
...
begin
  ...
  where barbershop_id = p_barbershop_id and enabled = true;
  ...
  select p_barbershop_id, v_member.id, v_settings.welcome_points, 'welcome', 'Pontos de boas-vindas'
  where v_settings.welcome_points > 0;
  ...
  if found then
    ...
    do update set points_balance = excluded.points_balance, lifetime_points = greatest(public.loyalty_accounts.lifetime_points, excluded.lifetime_points), updated_at = now();
  end if;
...
begin
  for v_row in
    select id, member_id, points_spent
  ...
  loop
    ...
    if v_row.member_id is not null then
      ...
      select r.barbershop_id, r.member_id, r.points_spent, 'refund', r.id, 'Resgate expirado'
...
begin
  ...
  update public.loyalty_members
  set points_balance = points_balance - v_reward.points_cost, updated_at = now()
  where id = v_member.id
  returning points_balance into v_new_balance;

  insert into public.loyalty_redemptions (
    barbershop_id, member_id, reward_id, points_spent, status, token_hash, code_hash, expires_at, created_at, updated_at
  )
  values (
    v_member.barbershop_id, v_member.id, v_reward.id, v_reward.points_cost, 'pending', p_token_hash, p_code_hash, p_expires_at, now(), now()
  )
...
begin
  ...
  if v_user.barbershop_id is distinct from p_barbershop_id then
    raise exception 'LOYALTY_STAFF_UNAUTHORIZED';
  end if;
  ...
  if v_redemption.expires_at is null or v_redemption.expires_at <= now() then
    ...
    insert into public.loyalty_member_transactions (barbershop_id, member_id, points, type, reference_id, description)
    values (v_redemption.barbershop_id, v_redemption.member_id, v_redemption.points_spent, 'refund', v_redemption.id, 'Resgate expirado');
    raise exception 'LOYALTY_REDEMPTION_EXPIRED';
  end if;

  select * into v_reward
  from public.loyalty_rewards
  where id = v_redemption.reward_id and barbershop_id = p_barbershop_id;
  if not found then raise exception 'LOYALTY_REWARD_NOT_FOUND'; end if;

  update public.loyalty_redemptions
  set status = 'fulfilled', validated_at = now(), validated_by_user_id = p_staff_user_id, fulfilled_at = now(), updated_at = now()
  where id = v_redemption.id;

  redemption_id := v_redemption.id;
  reward_name := v_reward.name;
  points_cost := v_redemption.points_spent;
