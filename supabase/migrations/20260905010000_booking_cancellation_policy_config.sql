...
alter table public.barbershops
  add column if not exists time_limit_cancellation_hours integer;

update public.barbershops
set time_limit_cancellation_hours = 24
where time_limit_cancellation_hours is null;

alter table public.barbershops
  alter column time_limit_cancellation_hours set default 24,
  alter column time_limit_cancellation_hours set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'barbershops_time_limit_cancellation_hours_check'
      and conrelid = 'public.barbershops'::regclass
  ) then
    alter table public.barbershops
      add constraint barbershops_time_limit_cancellation_hours_check
      check (time_limit_cancellation_hours between 0 and 720);
  end if;
end;
$$;
...
begin
  ...
  if p_config ? 'time_limit_cancellation_hours' then
    if (p_config->>'time_limit_cancellation_hours') !~ '^\\d+$' then
      raise exception 'invalid cancellation window';
    end if;

    v_cancellation_hours := (p_config->>'time_limit_cancellation_hours')::integer;
