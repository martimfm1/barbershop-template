...
create table if not exists public.barbershop_invite_codes (
  ...
  constraint barbershop_invite_codes_role_check check (role in ('admin', 'barber', 'receptionist'))
);
...
create or replace function public.create_barbershop_invite_code(
...
begin
  ...
  if p_role not in ('admin', 'barber', 'receptionist') then
  ...
  insert into public.barbershop_invite_codes (
    barbershop_id, code_hash, role, expires_at, created_by
  ) values (
    v_barbershop_id, v_hash, p_role, v_expires, v_user
  );
...
create or replace function public.join_barbershop_with_invite(
  p_code text
)
