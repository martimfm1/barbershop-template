...
export function useAppointments(
  ...
  const [bookingFormData, setBookingFormData] = useState({
    clientId: "", serviceId: "", professionalId: "", date: "", time: "", manualName: "", manualPhone: "", manualBirthDate: "",
  });
  ...
  const handleCreateBooking = useCallback(async (e?: React.SyntheticEvent) => {
    ...
    try {
      ...
      const { error } = await appointmentService.create({
        ...
        service_id: serviceId, professional_id: professionalId, manual_name: manualName || null, manual_phone: manualPhone || null,
  ...
  const finalizeBooking = useCallback(async (appointmentId: string, paymentMethod: string) => {
    ...
    try {
      ...
      const { error } = await appointmentService.update(appointmentId, { status: "completed", payment_method: paymentMethod, value_products: parsedValue, description_products: descriptionProducts });
      if (error) throw error;
      toast.success("Serviço concluído com sucesso."); setFinishingBookingId(null); setValueProducts(""); setDescriptionProducts(""); await onRefreshData();
  ...
  const addCompletedAppointmentClient = useCallback(async (appointmentId: string) => {
    ...
    try {
      ...
      toast.success(result.alreadyExists ? "Este cliente já estava na tua lista." : "Cliente adicionado à lista."); await onRefreshData();
  ...
  const handleCreateBlock = useCallback(async (e?: React.SyntheticEvent) => {
    ...
    setLoading(true);
    try {
      const { error } = await createScheduleBlock({ professional_id, barbershop_id: barbershopId, date: start_date, start_time, end_time, reason: reason || "Bloqueio de Agenda" });
      if (error) throw error; toast.success("Bloqueio de horário criado com sucesso."); setBlockFormData({ professional_id: "", start_date: "", start_time: "", end_time: "", reason: "" }); await onRefreshData();
  ...
  const handleDeleteBlock = useCallback(async (id: string) => {
    setLoading(true);
    try { const { error } = await deleteScheduleBlock(id); if (error) throw error; toast.success("Bloqueio de horário removido."); await onRefreshData(); }
    catch (error) { console.error("[Delete Block Hook Error]:", error); toast.error(getErrorMessage(error)); }
    finally { setLoading(false); }
  }, [onRefreshData]);

  const handleDeleteBooking = useCallback(async (id: string) => {
    setLoading(true);
    try { const { error } = await appointmentService.delete(id); if (error) throw error; toast.success("Marcação eliminada permanentemente."); await onRefreshData(); }
    catch (error) { console.error("[Delete Booking Hook Error]:", error); toast.error(getErrorMessage(error)); }
    finally { setLoading(false); }
  }, [onRefreshData]);

  return {
    loadingAppointments, finishingBookingId, setFinishingBookingId, addingClientAppointmentId, valueProducts, setValueProducts,
    descriptionProducts, setDescriptionProducts, bookingFormData, setBookingFormData, blockFormData, setBlockFormData,
    handleCreateBooking, confirmBooking, finalizeBooking, addCompletedAppointmentClient, handleCreateBlock, handleDeleteBlock, handleDeleteBooking,
  };
}
