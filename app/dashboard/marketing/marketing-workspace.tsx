...
function automationLabel(campaign: Campaign) {
  ...
  if (campaign.trigger_type === 'event') return EVENT_LABELS[campaign.event_name ?? ''] ?? campaign.event_name ?? 'Ação automática';
  if (campaign.trigger_type === 'birthday') {
    const offset = campaign.birthday_offset_days ?? 0;
    return offset === 0 ? 'No aniversário' : offset < 0 ? `${Math.abs(offset)} dia(s) antes do aniversário` : `${offset} dia(s) depois do aniversário`;
  }
  return 'Manual';
}
...
export function MarketingWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const nextCampaigns = (campaignData.campaigns ?? []) as Campaign[];
      const automationCampaigns = (automationData.campaigns ?? []) as Campaign[];
      const merged = nextCampaigns.map((campaign) => ({ ...campaign, ...(automationCampaigns.find((item) => item.id === campaign.id) ?? {}) }));
      setCampaigns(merged);
  ...
  const stats = useMemo(() => ({
    active: campaigns.filter((campaign) => campaign.active !== false && !['completed', 'cancelled'].includes(campaign.status)).length,
    recipients: campaigns.reduce((sum, campaign) => sum + Number(campaign.total_recipients ?? 0), 0),
    sent: campaigns.reduce((sum, campaign) => sum + Number(campaign.sent_count ?? 0), 0),
    failed: campaigns.reduce((sum, campaign) => sum + Number(campaign.failed_count ?? 0), 0),
  }), [campaigns]);
  ...
  function openEdit(campaign: Campaign) {
    if (!['draft', 'scheduled'].includes(campaign.status)) return toast.error('Esta campanha já não pode ser editada.');
  ...
  async function saveCampaign(event: FormEvent) {
    ...
    try {
      ...
      setCampaigns((current) => editorMode === 'edit' ? current.map((item) => item.id === data.campaign.id ? data.campaign : item) : [data.campaign, ...current]);
  ...
  if (!allowed) return <main className="min-h-screen bg-background px-4 py-20 sm:px-8"><Card className="mx-auto max-w-xl"><CardContent className="flex flex-col items-center gap-4 py-16 text-center"><Sparkles className="size-8 text-primary" aria-hidden="true" /><h1 className="text-2xl font-semibold">Marketing avançado</h1><p className="text-muted-foreground">Campanhas de email e SMS, automações e delivery num só lugar.</p><Button asChild><Link href="/dashboard/billing">Ver plano</Link></Button></CardContent></Card></main>;
