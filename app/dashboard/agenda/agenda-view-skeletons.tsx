...
function Shimmer({ className }: { className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={`relative block overflow-hidden rounded-lg bg-white/[0.055] ${className ?? ''}`}
    >
...
const ShimmerKeyframes = () => (
  <style>{`@keyframes agenda-shimmer{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}}`}</style>
);
...
export function AgendaLinesSkeleton() {
  return (
    <div
      className="space-y-4"
      role="status"
      aria-label="A carregar agenda em linhas"
    >
      <ShimmerKeyframes />
...
export function AgendaCalendarSkeleton() {
  return (
    <div
      ...
      role="status"
      aria-label="A carregar calendário"
    >
      <ShimmerKeyframes />
        ...
        <div className="p-3">
          ...
          <div className="mt-2 grid grid-cols-7 gap-1.5">
            {Array.from({ length: 42 }).map((_, index) => (
              <div
                key={index}
                className="min-h-24 rounded-xl border border-white/[0.045] bg-white/[0.018] p-1.5 sm:min-h-36 sm:p-3"
              >
