...
export default function LoyaltyPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSettings({ enabled: data.settings?.enabled !== false, points_per_euro: Number(data.settings?.points_per_euro ?? 1), welcome_points: Number(data.settings?.welcome_points ?? 0), referral_points: Number(data.settings?.referral_points ?? 0) });
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setSettings({ enabled: data.settings.enabled, points_per_euro: Number(data.settings.points_per_euro), welcome_points: Number(data.settings.welcome_points), referral_points: Number(data.settings.referral_points) });
      toast.success("Fidelização atualizada.");
  ...
  const saveReward = async () => {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Erro ao guardar recompensa."); }
    finally { setSaving(false); }
  };
  ...
  const deleteReward = async (id: string) => {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Erro ao desativar recompensa."); }
  };
