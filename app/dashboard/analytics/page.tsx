...
export default function AnalyticsPage() {
  ...
  return <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <Card><CardContent className="flex flex-col gap-4 py-4 sm:flex-row sm:items-end"><div className="flex-1"><label className="text-xs text-muted-foreground" htmlFor="analytics-from">De</label><Input id="analytics-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="mt-1" /></div><div className="flex-1"><label className="text-xs text-muted-foreground" htmlFor="analytics-to">Até</label><Input id="analytics-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} className="mt-1" /></div><Button onClick={() => void load()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />Atualizar</Button></CardContent></Card>
    ...
    {loading && !data ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Skeleton className="h-28" /><Skeleton className="h-28" /><Skeleton className="h-28" /><Skeleton className="h-28" /></div> : data && <>
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric title="Receita" value={money(data.overview.revenue)} icon={<Wallet className="h-4 w-4" />} change={data.overview.revenueChangePercent} /><Metric title="Marcações" value={String(data.overview.appointments)} icon={<CalendarDays className="h-4 w-4" />} /><Metric title="Clientes novos" value={String(data.overview.newClients)} icon={<Users className="h-4 w-4" />} /><Metric title="Cancelamentos" value={`${data.overview.cancellationRate.toFixed(1)}%`} icon={<CheckCircle2 className="h-4 w-4" />} /></section>
      <section className="grid gap-6 lg:grid-cols-[1.5fr_1fr]"><Card><CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5" />Receita diária</CardTitle></CardHeader><CardContent>{data.revenueByDay.length === 0 ? <Empty /> : <><div className="flex h-64 min-w-0 items-end gap-1 overflow-hidden">{data.revenueByDay.map((item) => <div key={item.date} className="group flex h-full min-w-0 flex-1 flex-col justify-end" title={`${item.date}: ${money(item.value)}`}><div className="min-h-1 rounded-t bg-primary/80 transition-colors group-hover:bg-primary" style={{ height: `${Math.max((item.value / maxRevenue) * 100, item.value ? 3 : 0)}%` }} /></div>)}</div><div className="mt-3 flex justify-between gap-4 text-xs text-muted-foreground"><span>{data.revenueByDay[0]?.date}</span><span>{data.revenueByDay.at(-1)?.date}</span></div></>}</CardContent></Card><Card><CardHeader><CardTitle>Resumo operacional</CardTitle></CardHeader><CardContent className="space-y-4"><Stat label="Concluídas" value={data.overview.completedAppointments} /><Stat label="Agendadas" value={data.overview.scheduledAppointments} /><Stat label="Canceladas" value={data.overview.cancelledAppointments} /><Stat label="Clientes ativos" value={data.overview.activeClientsInPeriod} /></CardContent></Card></section>
