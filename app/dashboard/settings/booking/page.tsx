      ...
      if (result.error) {
        toast.error(
          result.error.message ||
            'Não foi possível carregar as regras de marcações.',
        );
      } else {
        const next = Math.max(
          0,
        ...
        <header className="flex flex-col gap-4 border-b border-white/10 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
              Define com quantas horas de antecedência o cliente pode cancelar
              ou reagendar uma marcação através do portal.
            </p>
          </div>
          <Button
            onClick={() => void save()}
            disabled={!dirty || saving || hoursError}
            className="min-h-11 bg-white text-zinc-950 hover:bg-zinc-100"
          >
            {saving ? (
        ...
        <Card className="border-white/10 bg-white/[0.02]">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock3 className="size-4 text-violet-300" /> Prazo mínimo
            </CardTitle>
            <CardDescription>
              O valor aplica-se a toda a barbearia. Só owner e admin podem
          ...
          <CardContent className="space-y-6">
              ...
              <div className="grid gap-2">
                ...
                {hoursError ? (
                  ...
                  <p className="text-xs leading-5 text-zinc-600">
                    0 significa que não existe prazo mínimo.
                  </p>
                )}
              </div>
                ...
                {PRESETS.map((preset) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex min-h-10 items-center gap-1.5 rounded-xl border px-3 text-sm transition ${active ? 'border-violet-400/30 bg-violet-400/10 text-violet-200' : 'border-white/10 bg-white/[0.03] text-zinc-400 hover:bg-white/[0.06]'}`}
                    >
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              <p className="mt-2 text-xs leading-5 text-zinc-600">
                A validação é feita no servidor quando o cliente tenta cancelar
                ou reagendar, por isso a regra não pode ser contornada apenas
