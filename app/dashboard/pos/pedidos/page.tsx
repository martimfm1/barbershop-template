...
export default function MarketplaceOrdersPage() {
  ...
  return <main className="min-h-screen bg-zinc-950 px-4 py-8 text-zinc-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {loading ? <div className="grid gap-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-48 animate-pulse rounded-3xl bg-white/[0.03]" />)}</div> : filtered.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 p-14 text-center"><ClipboardList className="mx-auto size-7 text-zinc-700"/><p className="mt-4 text-sm text-zinc-500">Não há encomendas com estes filtros.</p></div> : <div className="space-y-4">
      {filtered.map((order) => { const meta = statusMeta[order.status]; const Icon = meta.icon; const next = order.status === 'pending' ? 'confirmed' : order.status === 'confirmed' ? 'ready' : order.status === 'ready' ? 'completed' : null; const isDelivery = order.fulfillment_method === 'delivery'; return <Card key={order.id} className="overflow-hidden border-white/10 bg-white/[0.02]">
        ...
        <CardContent className="space-y-4 p-4 sm:p-5"><div className="grid gap-3 lg:grid-cols-[1fr_280px]"><div className="space-y-2">{order.marketplace_order_items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.055] bg-white/[0.02] p-3 text-sm"><span className="min-w-0 truncate"><strong>{item.product_name}</strong><span className="ml-2 text-xs text-zinc-600">× {item.quantity}</span></span><span className="font-semibold">{money(item.total)}</span></div>)}{order.notes && <div className="rounded-xl border border-amber-400/10 bg-amber-400/[0.03] p-3 text-xs leading-5 text-amber-100"><strong>Nota:</strong> {order.notes}</div>}</div><div className="space-y-3"><div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-zinc-600">Cliente</p><a className="mt-2 flex items-center gap-2 text-sm text-zinc-200 hover:text-emerald-300" href={`mailto:${order.customer_email}`}><Mail className="size-3.5"/>{order.customer_email}</a><p className="mt-1 text-sm text-zinc-400">{order.customer_phone}</p></div>{isDelivery && <div className="rounded-2xl border border-sky-400/10 bg-sky-400/[0.03] p-4"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-sky-300"><MapPin className="size-3.5"/>Morada</p><p className="mt-2 text-sm leading-6 text-zinc-300">{order.shipping_address}<br/>{order.shipping_postal_code} {order.shipping_city}</p><p className="mt-2 text-[11px] leading-5 text-zinc-600">Trata do envio diretamente com o cliente.</p></div>}<div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><div className="flex items-center justify-between"><span className="text-sm text-zinc-500">Total</span><strong className="text-xl">{money(order.total)}</strong></div></div></div></div><div className="flex flex-col gap-2 border-t border-white/[0.06] pt-4 sm:flex-row sm:justify-end">{next && <Button disabled={updating === order.id} onClick={() => void changeStatus(order, next)} className="min-h-11 bg-emerald-400 text-zinc-950 hover:bg-emerald-300">{next === 'confirmed' ? 'Confirmar' : next === 'ready' ? (isDelivery ? 'Pronto para enviar' : 'Pronto para levantar') : 'Marcar como concluído'}</Button>}{order.status !== 'cancelled' && order.status !== 'completed' && <Button variant="outline" disabled={updating === order.id} onClick={() => void changeStatus(order, 'cancelled')} className="min-h-11 text-rose-300 hover:text-rose-200">Cancelar</Button>}</div></CardContent>
      </Card>; })}
    </div>}
  </div></main>;
}

function Summary({ icon: Icon, label, value, detail }: { icon: typeof Clock3; label: string; value: string; detail: string }) { return <Card className="border-white/10 bg-white/[0.02]"><CardContent className="p-4"><div className="flex items-center justify-between"><span className="text-xs text-zinc-500">{label}</span><Icon className="size-4 text-zinc-600"/></div><p className="mt-3 text-2xl font-semibold">{value}</p><p className="mt-1 text-[11px] text-zinc-600">{detail}</p></CardContent></Card>; }
