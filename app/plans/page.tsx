...
const faqs = [
  ...
  ["Como funciona o trial do Pro?", "O Barbers Pro inclui 14 dias de trial. Durante esse período tens acesso às funcionalidades Pro; depois inicia-se a cobrança do preço normal do plano, salvo cancelamento."],
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-16 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
        <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.12),transparent_34%),rgba(24,24,27,0.72)] p-5 shadow-[0_30px_90px_rgba(0,0,0,0.24)] sm:p-8 lg:p-10">
          ...
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">Começa no Free e, quando fizer sentido, experimenta o Pro durante 14 dias. O plano pertence à barbearia, não a cada utilizador.</p>
            <div className="mt-6 grid gap-2 text-xs text-zinc-500 sm:flex sm:flex-wrap sm:gap-x-5">
              <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5 text-emerald-300" />Free disponível sem subscrição paga</span>
              <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5 text-emerald-300" />14 dias de Pro</span>
              <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5 text-emerald-300" />Membros da equipa herdam o plano</span>
              <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5 text-emerald-300" />Faturação com Stripe</span>
            </div>
          </div>
        </section>
        ...
        <section className="mt-10 rounded-[2rem] border border-white/10 bg-white/[0.02] p-5 sm:mt-14 sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link href="/registo" className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 bg-white px-4 text-sm font-semibold text-zinc-950">Começar grátis <ArrowRight className="size-4" /></Link>
          </div>
          <div className="mt-7 overflow-x-auto"><PlanComparison /></div>
        </section>

        <section className="mt-10 sm:mt-14"><div className="max-w-2xl"><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300/80">Perguntas frequentes</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.045em] text-white sm:text-4xl">Antes de escolher, tira as dúvidas principais.</h2></div><div className="mt-7 grid gap-3 md:grid-cols-2">{faqs.map(([question, answer]) => <article key={question} className="border border-white/10 bg-zinc-900/45 p-5 transition hover:border-white/20 hover:bg-white/[0.035]"><CircleHelp className="size-5 text-emerald-300" aria-hidden="true" /><h2 className="mt-4 font-semibold text-zinc-100">{question}</h2><p className="mt-2 text-sm leading-6 text-zinc-400">{answer}</p></article>)}</div></section>

        <section className="mt-10 border border-white/10 bg-white/[0.03] p-5 sm:mt-14 sm:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300/80">Decisão simples</p><p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-white">Começa no Free. Experimenta o Pro durante 14 dias.</p><p className="mt-2 text-sm leading-6 text-zinc-500">Podes gerir o plano e as faturas a partir das definições da tua barbearia.</p></div><Link href="/registo" className="inline-flex min-h-12 items-center justify-center gap-2 bg-white px-5 text-sm font-semibold text-zinc-950">Criar barbearia <ArrowRight className="size-4" /></Link></div></section>
      </main>
    </div>
  );
}
