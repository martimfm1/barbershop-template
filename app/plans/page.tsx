  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-zinc-950 text-zinc-50 antialiased">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-16 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
        ...
        <section className="mt-8 sm:mt-12"><PricingSection destination="checkout" /></section>

        <section className="mt-10 rounded-[2rem] border border-white/10 bg-white/[0.02] p-5 sm:mt-14 sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Comparação completa</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-white sm:text-3xl">Vê exatamente o que muda entre planos.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">Compara funcionalidades e limites antes de iniciar o checkout.</p></div>
        ...
        <section className="mt-10 border border-white/10 bg-white/[0.03] p-5 sm:mt-14 sm:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300/80">Próximo passo</p><p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-white">Escolhe um plano e continua para o checkout.</p><p className="mt-2 text-sm leading-6 text-zinc-500">O pagamento é processado pela Stripe dentro da experiência Silentra.</p></div><Link href="#precos" className="inline-flex min-h-12 items-center justify-center gap-2 bg-white px-5 text-sm font-semibold text-zinc-950">Escolher plano <ArrowRight className="size-4" /></Link></div></section>
      </main>
    </div>
  );
}
