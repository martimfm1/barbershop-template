...
export async function POST(request: Request) {
  try {
    ...
    if (typeof inviteCode !== "string" || inviteCode.trim().length < 6) return NextResponse.json({ error: "Introduz um código de convite válido." }, { status: 400 });
    ...
    if (!membership?.barbershop_id || !membership?.role) {
      console.error("[ONBOARDING_JOIN_INVALID_RESPONSE]", "INVALID_RPC_RESPONSE");
      return NextResponse.json({ error: "Não foi possível concluir a associação." }, { status: 500 });
    }

    return NextResponse.json({ success: true, barbershopId: membership.barbershop_id, role: membership.role });
  } catch (error) {
    console.error("[ONBOARDING_JOIN_CRITICAL_ERROR]", error instanceof Error ? error.name : "UNKNOWN");
