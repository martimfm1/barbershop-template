...
export async function GET() {
  try {
    ...
    if (userError) {
      console.error("[BILLING_SUMMARY_USER_ERROR]", userError);
    ...
    if (subscriptionResult.error) {
      console.error("[BILLING_SUMMARY_SUBSCRIPTION_ERROR]", subscriptionResult.error);
      return NextResponse.json({ error: "Could not load subscription." }, { status: 500 });
    }
    if (assignmentResult.error) {
      console.error("[BILLING_SUMMARY_ASSIGNMENT_ERROR]", assignmentResult.error);
      return NextResponse.json({ error: "Could not load plan assignment." }, { status: 500 });
    }
    if (barbershopResult.error) {
      console.error("[BILLING_SUMMARY_BARBERSHOP_ERROR]", barbershopResult.error);
      return NextResponse.json({ error: "Could not load barbershop." }, { status: 500 });
    }
    ...
    console.error("[BILLING_SUMMARY_CRITICAL]", error);
