...
export async function POST() {
  try {
    ...
    if (!owner?.barbershop_id || String(owner.role ?? "").toLowerCase() !== "owner") {
      throw new BillingError("Only the barbershop owner can change the subscription.", "SUBSCRIPTION_NOT_ACTIVE");
    }
    ...
    if (accountError) throw new BillingError("Could not load Stripe billing account.", "DB_READ_FAILED");
    if (!account?.stripe_customer_id) return NextResponse.json({ success: true, changed: false });
    ...
    if (previous && ["active", "trialing", "past_due", "unpaid", "incomplete"].includes(previous.status)) {
    ...
    return NextResponse.json({ error: "Could not finalize the plan change." }, { status: 500 });
  }
}
