...
const EVENTS = ['booking_created', 'booking_completed', 'booking_cancelled'] as const;
...
export async function GET() {
  try {
    ...
    const { data, error } = await admin
      ...
      .select('id,name,channel,trigger_type,interval_value,interval_unit,next_run_at,event_name,active,status')
      .eq('barbershop_id', barbershopId)
      .order('created_at', { ascending: false });
    if (error) throw error;
    return NextResponse.json({ ok: true, campaigns: data ?? [], events: EVENTS });
  } catch (error) {
    const response = moduleErrorResponse(error);
    if (response) return response;
    return NextResponse.json({ ok: false, error: 'Unable to load campaign automation settings.' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!id || !['manual', 'interval', 'event'].includes(mode)) {
    ...
    if (mode === 'interval') {
      ...
      if (!Number.isInteger(value) || value < 1 || value > 3650 || !['hours', 'days'].includes(String(unit))) {
      ...
      patch.active = true;
      patch.next_run_at = new Date(Date.now() + (unit === 'days' ? value * 86400000 : value * 3600000)).toISOString();
    ...
    return NextResponse.json({ ok: false, error: 'Não foi possível atualizar a automação.' }, { status: 500 });
  }
}
