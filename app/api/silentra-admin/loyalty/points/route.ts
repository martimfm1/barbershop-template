...
export async function POST(request: Request) {
  try {
    ...
    if (!Number.isInteger(points) || points <= 0 || points > 1000000) {
      return NextResponse.json({ error: "Os pontos devem ser um inteiro entre 1 e 1.000.000." }, { status: 400 });
    }
    if (!reason || reason.length > 500) {
      return NextResponse.json({ error: "Indica um motivo (máx. 500 caracteres)." }, { status: 400 });
    }

    const { data, error } = await admin.rpc("admin_grant_loyalty_points_by_email", {
      p_email: email,
      p_points: points,
      p_reason: reason,
    });
    ...
    if (error) {
      if (error.message === "LOYALTY_MEMBER_NOT_FOUND") {
        return NextResponse.json({ error: "Não existe um membro ativo de fidelização com esse email." }, { status: 404 });
      }
