...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("admin_find_loyalty_member_by_email", { p_email: email });
    ...
    return NextResponse.json({ error: "Não foi possível pesquisar a fidelização." }, { status: 500 });
  }
}
