...
export async function GET(request: Request) {
  ...
  if (!expectedSecret) {
    ...
    return NextResponse.json({ error: "Cron is not configured", requestId }, { status: 503 });
  }
  ...
  for (const automation of automations ?? []) {
    ...
    const [{ data: shop, error: shopError }, { data: clients, error: clientError }] = await Promise.all([
    ...
    for (const client of clients) {
      ...
      try {
        ...
        if (result.success) {
          sent++;
          const { error: logError } = await admin.from("birthday_email_logs").upsert({
