...
export async function GET(request: Request) {
  try {
    ...
    const from = startOfDay(parseDate(url.searchParams.get("from"), defaultFrom));
    ...
    const scheduled = rows.filter((row) => row.status === "scheduled" || row.status === "completed");
    ...
    for (const row of completed) {
      const servicePrice = Number(row.service?.price ?? 0);
      const productValue = Number(row.value_products ?? 0);
      const revenue = servicePrice + (Number.isFinite(productValue) ? productValue : 0);
      const day = row.date_hour.slice(0, 10);
      revenueByDay.set(day, money((revenueByDay.get(day) ?? 0) + revenue));
    ...
    const revenue = money(completed.reduce((sum, row) => {
      const servicePrice = Number(row.service?.price ?? 0);
      const productValue = Number(row.value_products ?? 0);
      return sum + servicePrice + (Number.isFinite(productValue) ? productValue : 0);
    }, 0));
    ...
    const response: Record<string, unknown> = {
      ...
      overview: {
        ...
        cancellationRate: rows.length === 0 ? 0 : money((cancelled.length / rows.length) * 100),
      ...
      topServices: Array.from(serviceStats.entries()).map(([name, stats]) => ({ name, ...stats })).sort((a, b) => b.revenue - a.revenue).slice(0, 10),
      professionals: Array.from(professionalStats.entries()).map(([name, stats]) => ({ name, ...stats })).sort((a, b) => b.revenue - a.revenue),
    };
    ...
    if (plan === PLANS.ENTERPRISE) {
      ...
      const posRevenue = money(posRows.filter((row) => row.status === "completed").reduce((sum, row) => sum + Number(row.total), 0));
      const locationStats = new Map<string, { transactions: number; revenue: number }>();
      for (const row of posRows.filter((item) => item.status === "completed")) {
      ...
      response.enterprise = {
        posRevenue,
        posTransactions: posRows.filter((row) => row.status === "completed").length,
