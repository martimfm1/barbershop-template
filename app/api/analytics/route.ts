...
type AppointmentRow = { id: string; date_hour: string; status: string; value_products: number | string | null; service: { name: string; price: number | string } | null; professional: { name: string } | null; client_id: string | null; manual_birth_date: string | null; user: { birth_date: string | null } | null };
...
function calculateAge(birthDate: string, referenceDate: Date) { const birth = new Date(`${birthDate}T00:00:00`); if (Number.isNaN(birth.getTime()) || birth > referenceDate) return null; let age = referenceDate.getFullYear() - birth.getFullYear(); const month = referenceDate.getMonth() - birth.getMonth(); if (month < 0 || (month === 0 && referenceDate.getDate() < birth.getDate())) age -= 1; return age >= 0 && age <= 120 ? age : null; }
function ageGroup(age: number) { if (age < 18) return "Menos de 18"; if (age <= 24) return "18–24"; if (age <= 34) return "25–34"; if (age <= 44) return "35–44"; if (age <= 54) return "45–54"; if (age <= 64) return "55–64"; return "65+"; }
...
export async function GET(request: Request) {
  try {
    ...
    const { data: appointments, error: appointmentsError } = await admin.from("appointments").select("id,date_hour,status,value_products,client_id,manual_birth_date,service:services(name,price),professional:professionals(name),user:users(birth_date)").eq("barbershop_id", barbershopId).gte("date_hour", from.toISOString()).lte("date_hour", to.toISOString());
    if (appointmentsError) throw appointmentsError;
    const rows = (appointments ?? []) as unknown as AppointmentRow[];
    const completed = rows.filter((row) => row.status === "completed"); const scheduled = rows.filter((row) => row.status === "scheduled"); const cancelled = rows.filter((row) => row.status === "cancelled");
    const revenueByDay = new Map<string, number>(); const serviceStats = new Map<string, { bookings: number; revenue: number }>(); const professionalStats = new Map<string, { bookings: number; revenue: number }>(); const clientIds = new Set<string>(); const clientAgeGroups = new Map<string, Set<string>>();

    for (const row of completed) {
      const revenue = Number(row.service?.price ?? 0) + Number(row.value_products ?? 0); const safeRevenue = Number.isFinite(revenue) ? revenue : 0; const day = row.date_hour.slice(0, 10);
      ...
      if (row.client_id) { clientIds.add(row.client_id); const birthDate = row.user?.birth_date ?? row.manual_birth_date; if (birthDate) { const age = calculateAge(birthDate, new Date(row.date_hour)); if (age !== null) { const group = ageGroup(age); const set = clientAgeGroups.get(group) ?? new Set<string>(); set.add(row.client_id); clientAgeGroups.set(group, set); } } }
    }
    ...
    const previousTo = new Date(from.getTime() - 1); const previousFrom = new Date(previousTo.getTime() - (to.getTime() - from.getTime()));
    ...
    const response: Record<string, unknown> = { plan, period: { from: from.toISOString(), to: to.toISOString(), previousFrom: previousFrom.toISOString(), previousTo: previousTo.toISOString() }, overview: { revenue, previousRevenue, revenueChangePercent: previousRevenue === 0 ? null : money(((revenue - previousRevenue) / previousRevenue) * 100), appointments: rows.length, completedAppointments: completed.length, scheduledAppointments: scheduled.length, cancelledAppointments: cancelled.length, cancellationRate: rows.length === 0 ? 0 : money((cancelled.length / rows.length) * 100), newClients: newClients ?? 0, totalClients: totalClients ?? 0, activeClientsInPeriod: clientIds.size }, revenueByDay: Array.from(revenueByDay.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([date, value]) => ({ date, value })), topServices: Array.from(serviceStats.entries()).map(([name, stats]) => ({ name, count: stats.bookings, revenue: stats.revenue })).sort((a, b) => b.revenue - a.revenue).slice(0, 10), professionals: Array.from(professionalStats.entries()).map(([name, stats]) => ({ name, appointments: stats.bookings, revenue: stats.revenue })).sort((a, b) => b.revenue - a.revenue), clientAgeGroups: ageInsights };
    ...
    if (plan === PLANS.ENTERPRISE) {
      ...
      if (posError) throw posError; const posRows = (posTransactions ?? []) as PosRow[]; const completedPos = posRows.filter((row) => row.status === "completed"); const posRevenue = money(completedPos.reduce((sum, row) => sum + Number(row.total), 0)); const locationStats = new Map<string, { transactions: number; revenue: number }>();
  ...
  } catch (error) { const response = moduleErrorResponse(error); if (response) return response; console.error("Analytics API error", error); return NextResponse.json({ error: "Unable to load analytics" }, { status: 500 }); }
}
