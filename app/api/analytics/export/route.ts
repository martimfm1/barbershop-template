  ...
  try {
    ...
    if (type === 'clients') {
      const { data, error } = await admin
        .from('users')
        .select('id,name_complete,email,num_phone,birth_date,created_at,role')
      ...
      const lines = [
        ['id', 'name', 'email', 'phone', 'birth_date', 'created_at'],
        ...(data ?? []).map((row) => [
          row.id,
          row.name_complete,
          row.email,
      ...
      return new NextResponse(lines.join('\n'), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="silentra-clients-${from.toISOString().slice(0, 10)}-${to.toISOString().slice(0, 10)}.csv"`,
    ...
    if (type === 'appointments') {
      const { data, error } = await admin
        .from('appointments')
        .select(
          'id,date_hour,status,manual_name,manual_email,manual_phone,value_products,payment_method,service:services(name,price),professional:professionals(name)',
        )
        .eq('barbershop_id', barbershopId)
      ...
      return new NextResponse(lines.join('\n'), {
        headers: {
          'Content-Type': 'text/csv; charset=utf-8',
          'Content-Disposition': `attachment; filename="silentra-appointments-${from.toISOString().slice(0, 10)}-${to.toISOString().slice(0, 10)}.csv"`,
          'Cache-Control': 'no-store',
        },
      });
    }
    ...
    if (type === 'pos') {
      ...
      const { data, error } = await admin
        ...
        .select(
          'id,created_at,subtotal,discount,total,payment_method,status,location_id,appointment_id,client_id',
        )
        .eq('barbershop_id', barbershopId)
      ...
      return new NextResponse(lines.join('\n'), {
        headers: {
          'Content-Type': 'text/csv; charset=utf-8',
          'Content-Disposition': `attachment; filename="silentra-pos-${from.toISOString().slice(0, 10)}-${to.toISOString().slice(0, 10)}.csv"`,
          'Cache-Control': 'no-store',
        },
      });
    }
