...
export async function POST(request: Request) {
  try {
    ...
    if (!EMAIL_PATTERN.test(email) || email.length > 254) return genericSuccessResponse();
    ...
    if (!supabaseUrl || !serviceRoleKey || !apiKey || !senderEmail) {
      console.error("[AUTH_RECOVERY_CONFIG_ERROR] Missing required server configuration.");
      return NextResponse.json({ error: "O serviÃ§o de recuperaÃ§Ã£o estÃ¡ temporariamente indisponÃ­vel." }, { status: 503 });
    }
    ...
    const { data, error } = await supabaseAdmin.auth.admin.generateLink({
      type: "recovery",
      email,
      options: { redirectTo: `${getAuthCallbackUrl(request)}?next=/reset-password` },
    });

    if (error) {
      console.warn("[AUTH_RECOVERY_LINK_REJECTED]", { code: error.code, status: error.status });
    ...
    if (!response.ok) {
      console.error("[AUTH_RECOVERY_EMAIL_REJECTED]", { status: response.status });
      return NextResponse.json({ error: "NÃ£o foi possÃ­vel enviar o email de recuperaÃ§Ã£o. Tenta novamente." }, { status: 502 });
    }

    return genericSuccessResponse();
  } catch (error) {
    console.error("[AUTH_RECOVERY_ERROR]", error instanceof Error ? error.name : "unknown");
    return NextResponse.json({ error: "Ocorreu um erro interno ao processar o pedido." }, { status: 500 });
  }
}
