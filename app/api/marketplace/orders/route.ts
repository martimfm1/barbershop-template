...
function buildOrderReceiptEmail(input: {
  ...
  const itemsHtml = input.items
    .map(
      (item) => `
        <tr>
          <td style="padding:12px 0;border-bottom:1px solid #27272a;color:#f4f4f5;">${escapeHtml(item.product_name)} <span style="color:#71717a;">× ${item.quantity}</span></td>
          <td style="padding:12px 0;border-bottom:1px solid #27272a;text-align:right;color:#f4f4f5;">${formatMoney(Number(item.total))}</td>
        </tr>`,
    )
    .join('');
...
export async function POST(request: Request) {
  try {
    ...
    if (!FULFILLMENT_METHODS.includes(fulfillmentMethod as FulfillmentMethod)) {
      return NextResponse.json({ error: 'Escolhe uma forma de entrega válida.' }, { status: 400 });
    }
    if (fulfillmentMethod === 'delivery' && (!shippingAddress || !shippingCity || !shippingPostalCode)) {
      return NextResponse.json({ error: 'Preenche a morada de entrega.' }, { status: 400 });
    }
    ...
    void sendBrevoEmail({
      ...
      htmlContent: buildOrderReceiptEmail({
        ...
        items: itemsForEmail.map((item: { product_name: string; quantity: number; unit_price: number; total: number }) => ({
      ...
      if (!result.success) console.error('[MARKETPLACE_RECEIPT_EMAIL_FAILED]', { orderId: order.id, error: result.error });
    });
