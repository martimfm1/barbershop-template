...
import { isSafePublicBookingDate, isValidTime, UUID_PATTERN } from "@/lib/validation";
...
async function getAuthorizedAppointment(appointmentId: string) {
  ...
  if (!session || !UUID_PATTERN.test(appointmentId)) return { session, appointment: null, admin: null };
...
function getShop(appointment: any) {
  return Array.isArray(appointment.barbershops) ? appointment.barbershops[0] : appointment.barbershops;
}

async function getAvailability(admin: ReturnType<typeof createAdminClient>, appointment: any, date: string) {
  const shop = getShop(appointment);
  const isClosedDay = parseClosedDays(shop?.closed_days).has(new Date(`${date}T12:00:00`).getDay());
  ...
  if (isClosedDay) return { availableSlots: [], closedDay: true, blockedIntervals: [] };
  ...
  if (blocksError && blocksError.code !== "42P01") throw new Error("BLOCK_LOOKUP_FAILED");

  const visibleBlocks = (blocks ?? []).filter((block: any) =>
    !block.professional_id || block.professional_id === appointment.professional_id,
  );
  ...
  const blockedIntervals = visibleBlocks.map((block: any) => ({
    ...
    reason: String(block.reason || "Horário bloqueado").trim() || "Horário bloqueado",
  }));

  const slots: string[] = [];
  for (let start = timeToMinutes(openingTime); start + duration <= timeToMinutes(closingTime); start += 30) {
    const end = start + duration;
    if (lunchStart !== null && lunchEnd !== null && overlaps(start, end, lunchStart, lunchEnd)) continue;
    if (visibleBlocks.some((block: any) => !block.start_time || !block.end_time || overlaps(start, end, timeToMinutes(block.start_time), timeToMinutes(block.end_time)))) continue;
    if (occupied.some((item) => overlaps(start, end, item.start, item.end))) continue;
    slots.push(minutesToTime(start));
  }

  const todayPortugal = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Lisbon" }).format(new Date());
  if (date === todayPortugal) {
    const currentMinutes = timeToMinutes(new Intl.DateTimeFormat("pt-PT", { timeZone: "Europe/Lisbon", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date()));
    return {
      availableSlots: slots.filter((slot) => timeToMinutes(slot) > currentMinutes),
      closedDay: false,
      blockedIntervals,
    };
  }
...
export async function GET(
  ...
  try {
    ...
    if (!isSafePublicBookingDate(date)) return jsonError("Indica uma data válida.");
...
export async function DELETE(
  ...
  try {
    ...
    if (!updated) return jsonError("A marcação já foi alterada. Atualiza a página e tenta novamente.", 409);
...
export async function PATCH(
  ...
  try {
    ...
    if (!isSafePublicBookingDate(date) || !isValidTime(slot)) return jsonError("Indica uma data e hora válidas.");
