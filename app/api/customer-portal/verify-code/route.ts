...
function rateLimitKey(request: Request, email: string): string {
  ...
  return createHmac("sha256", secret).update(`customer-portal-verify:${email}:${getClientIp(request)}`).digest("hex");
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!email || !/^\d{6}$/.test(code)) return NextResponse.json({ success: false, error: "Email ou código inválido." }, { status: 400 });
    ...
    if (rateError) {
      ...
      return NextResponse.json({ success: false, error: "Não foi possível processar o código neste momento." }, { status: 503 });
    }
    if (allowed !== true) return NextResponse.json({ success: false, error: "Demasiadas tentativas. Pede um novo código." }, { status: 429 });

    const { data: verification, error } = await admin.rpc("verify_booking_portal_code", {
      p_email: email,
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: "Não foi possível verificar o código." }, { status: 503 });
    }
    if (!result?.ok) {
      return NextResponse.json(
        { success: false, error: result?.error_code === "TOO_MANY_ATTEMPTS" ? "Demasiadas tentativas. Pede um novo código." : "Código inválido ou expirado." },
    ...
    if (revokeError) console.warn("[CUSTOMER_PORTAL_SESSION_CLEANUP_ERROR]", revokeError);
