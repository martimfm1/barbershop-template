...
async function getTenant(req: Request) {
  ...
  if (!shop) return null;
  return { admin, barbershopId: user.barbershop_id, shopName: shop.name ?? "Silentra" };
}
...
export async function POST(req: Request) {
  ...
  if (!clientId || !["reminder", "thanks", "custom"].includes(templateKey)) {
    return NextResponse.json({ error: "Destinatário ou template inválido." }, { status: 400 });
  }
  ...
  if (!client?.email) {
    return NextResponse.json({ error: "Este cliente não tem um endereço de email válido." }, { status: 400 });
  }

  const clientName = client.name_complete || client.name || "Cliente";
  const template = getTemplate(templateKey as keyof typeof TEMPLATES, clientName, tenant.shopName);
  ...
  if (!subject || !body || subject.length > MAX_SUBJECT_LENGTH || body.length > MAX_BODY_LENGTH) {
    return NextResponse.json({ error: "Assunto ou mensagem inválidos." }, { status: 400 });
  }
  ...
  const htmlContent = `<!doctype html><html lang="pt"><body style="margin:0;padding:32px 16px;background:#09090b;color:#f4f4f5;font-family:Arial,sans-serif"><div style="max-width:600px;margin:auto;padding:28px;border:1px solid #27272a;border-radius:14px;background:#18181b"><p style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#a1a1aa;font-weight:700">${escapeHtml(tenant.shopName)}</p><div style="white-space:pre-wrap;font-size:15px;line-height:1.7;color:#e4e4e7">${escapeHtml(body)}</div><p style="margin-top:28px;font-size:12px;color:#71717a">Mensagem enviada através do Silentra.</p></div></body></html>`;
  ...
  try {
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: { name: tenant.shopName, email: senderEmail },
