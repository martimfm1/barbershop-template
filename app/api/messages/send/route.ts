...
function getTemplate(key: keyof typeof TEMPLATES, name: string, shopName: string) {
  const template = TEMPLATES[key];
  return {
    subject: template.subject.replaceAll("{{barbearia}}", shopName).replaceAll("{{nome}}", name),
    body: template.body.replaceAll("{{barbearia}}", shopName).replaceAll("{{nome}}", name),
  };
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!payload || typeof payload !== "object") return NextResponse.json({ error: "Pedido inválido." }, { status: 400 });

    const input = payload as Record<string, unknown>;
    const clientId = typeof input.clientId === "string" ? input.clientId : "";
    const templateKey = typeof input.template === "string" ? input.template : "custom";
    const subjectInput = typeof input.subject === "string" ? input.subject.trim() : "";
    const bodyInput = typeof input.body === "string" ? input.body.trim() : "";

    if (!clientId || !["reminder", "thanks", "custom"].includes(templateKey)) return NextResponse.json({ error: "Destinatário ou template inválido." }, { status: 400 });
    ...
    const { data: client } = await tenant.admin
      ...
      .eq("role", "client")
      .maybeSingle();
    if (!client?.email) return NextResponse.json({ error: "Este cliente não tem um endereço de email válido." }, { status: 400 });

    const clientName = client.name_complete || client.name || "Cliente";
    const shop = await tenant.admin.from("barbershops").select("name").eq("id", tenant.barbershopId).maybeSingle();
    const shopName = shop.data?.name || "Silentra";
    const template = getTemplate(templateKey as keyof typeof TEMPLATES, clientName, shopName);
    const subject = (templateKey === "custom" ? subjectInput : template.subject).trim();
    const body = (templateKey === "custom" ? bodyInput : template.body).trim();
    if (!subject || !body || subject.length > MAX_SUBJECT_LENGTH || body.length > MAX_BODY_LENGTH) return NextResponse.json({ error: "Assunto ou mensagem inválidos." }, { status: 400 });

    const apiKey = process.env.BREVO_API_KEY;
    const senderEmail = process.env.BREVO_FROM_EMAIL;
    const senderName = process.env.BREVO_FROM_NAME || shopName;
    if (!apiKey || !senderEmail) return NextResponse.json({ error: "O envio de email ainda não está configurado." }, { status: 503 });
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({ sender: { name: senderName, email: senderEmail }, to: [{ email: client.email, name: clientName }], subject, htmlContent }),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error("[MESSAGES_BREVO_ERROR]", data);
      return NextResponse.json({ error: "O Brevo recusou o envio da mensagem." }, { status: 502 });
    }

    await tenant.admin.from("audit_logs").insert({ action: "manual_email_sent", entity_type: "user", entity_id: client.id, metadata: { template: templateKey, subject } });
