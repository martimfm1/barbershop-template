  ...
  try {
    ...
    if (!barbershopId || !PLANS.has(plan)) {
      return NextResponse.json(
        { error: 'Barbearia e plano válidos são obrigatórios.' },
        { status: 400 },
      );
    }
    ...
    if (expiresAt && Number.isNaN(Date.parse(expiresAt))) {
      return NextResponse.json(
        { error: 'A data de expiração é inválida.' },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: 'Não foi possível atribuir o plano.' },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: 'Não foi possível remover a atribuição.' },
      { status: 500 },
    );
  }
}
