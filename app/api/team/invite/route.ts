...
const ALLOWED_ROLES = new Set(["admin", "manager", "barber", "receptionist"]);
...
export async function POST(request: Request) {
  try {
    ...
    if (!ALLOWED_ROLES.has(role)) return NextResponse.json({ error: "Função inválida" }, { status: 400 });

    const { data, error } = await supabase.rpc("create_barbershop_invite_code", { p_role: role });
    if (error) {
      ...
      if (error.code === "22023") return NextResponse.json({ error: "Função inválida" }, { status: 400 });
    ...
    return NextResponse.json({ code: invite.code, role: invite.role, expiresAt: invite.expires_at });
  } catch (error) {
