...
export async function PATCH(request: Request) {
  ...
  if (!tenant.ok) {
    return NextResponse.json(
      { error: tenant.status === 401 ? "Sessão inválida." : "Sem permissão para alterar o avatar." },
      { status: tenant.status },
    );
  }

  const body: unknown = await request.json().catch(() => null);
  const avatarUrl = isRecord(body) && typeof body.avatarUrl === "string"
    ? body.avatarUrl.trim()
    : "";
  ...
  try {
    ...
    return NextResponse.json({ error: "O endereço do avatar é inválido." }, { status: 400 });
  }

  if (parsedUrl.pathname !== expectedPath) {
    return NextResponse.json({ error: "O endereço do avatar não pertence à tua barbearia." }, { status: 400 });
  }
  ...
  if (error) {
    console.error("[BARBERSHOP_AVATAR_UPDATE_FAIL]", {
      code: error.code,
