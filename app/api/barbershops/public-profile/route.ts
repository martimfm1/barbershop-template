...
export async function GET() {
  try {
    ...
    const { data: shop, error: shopError } = await database
      .from('shops')
      .select(
        'id, slug, custom_slug, public_profile_enabled, seo_title, seo_description, og_image_url, theme_config',
      )
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!canCustomizeSlug)
      return jsonError(
        'A personalização da presença online está disponível a partir do plano Pro.',
        403,
      );
