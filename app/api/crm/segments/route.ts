...
function parseSegment(value: string | null): Segment {
  return value === "inactive" || value === "frequent" || value === "high_value" || value === "new" ? value : "all";
}

function parseDays(value: string | null): number {
  const days = Number(value ?? DEFAULT_DAYS);
  return Number.isInteger(days) && days >= 7 && days <= 3650 ? days : DEFAULT_DAYS;
}
...
export async function GET(request: Request) {
  try {
    ...
    if (staffError || !staff?.barbershop_id || !["admin", "owner", "staff"].includes(staff.role ?? "staff")) {
    ...
    const clientIds = (clients ?? []).map((client) => client.id).filter((id): id is string => UUID_PATTERN.test(id));
    ...
    const stats = new Map<string, { visits: number; spent: number; lastVisit: number | null }>();

    for (const appointment of appointments ?? []) {
      if (!appointment.client_id) continue;
      const current = stats.get(appointment.client_id) ?? { visits: 0, spent: 0, lastVisit: null };
    ...
    const result = (clients ?? []).map((client) => {
      const current = stats.get(client.id) ?? { visits: 0, spent: 0, lastVisit: null };
      return { ...client, ...current };
    }).filter((client) => {
      if (search) {
        const haystack = [client.name_complete, client.name, client.email, client.num_phone].filter(Boolean).join(" ").toLowerCase();
        if (!haystack.includes(search.toLowerCase())) return false;
      }
      if (segment === "inactive") return !client.lastVisit || client.lastVisit < cutoff;
    ...
    return NextResponse.json({ data: result, meta: { segment, days, total: result.length } });
  } catch (error) {
    console.error("[CRM_SEGMENTS_GET]", error);
    return NextResponse.json({ error: "Não foi possível carregar os segmentos.", code: "INTERNAL_ERROR" }, { status: 500 });
  }
}
