...
export async function GET(req: Request) {
  try {
    ...
    if (search) {
      ...
      if (escaped) query = query.or(`name_complete.ilike.%${escaped}%,name.ilike.%${escaped}%,email.ilike.%${escaped}%,num_phone.ilike.%${escaped}%`);
    }
    ...
    return NextResponse.json({ clients: data ?? [], total: count ?? 0, limit, offset }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
