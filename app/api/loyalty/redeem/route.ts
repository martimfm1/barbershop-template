...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!slug || !rewardId) return errorResponse("Recompensa inválida.", 400, { requestId });

    const tenant = await getLoyaltyTenantBySlug(slug);
    tenantId = tenant?.barbershopId ?? null;
    if (!tenant) return errorResponse("Fidelização indisponível.", 404, { requestId });
    ...
    if (!session) return errorResponse("Sessão expirada. Confirma novamente o teu email.", 401, { requestId });

    const admin = createAdminClient();
    const [{ data: enabledSettings, error: settingsError }, { data: barbershop, error: barbershopError }] = await Promise.all([
    ...
    if (memberError || !member) return errorResponse(memberError ? "Não foi possível carregar a adesão." : "Não tens uma adesão ativa nesta barbearia.", memberError ? 503 : 409, { requestId });

    const { data: reward, error: rewardLookupError } = await admin.from("loyalty_rewards").select("id, name, description, points_cost, reward_type, reward_value, active").eq("id", rewardId).eq("barbershop_id", tenant.barbershopId).eq("active", true).maybeSingle();
    if (rewardLookupError) return errorResponse("Não foi possível carregar a recompensa.", 503, { requestId });
    if (!reward) return errorResponse("Esta recompensa já não está disponível.", 404, { requestId });
    if (member.points_balance < reward.points_cost) return errorResponse("Ainda não tens pontos suficientes para esta recompensa.", 409, { requestId });

    const { data: existingRedemption, error: existingError } = await admin.from("loyalty_redemptions").select("id").eq("member_id", member.id).eq("status", "pending").gt("expires_at", new Date().toISOString()).limit(1).maybeSingle();
    if (existingError) return errorResponse("Não foi possível validar os teus resgates pendentes.", 503, { requestId });
    if (existingRedemption) return errorResponse("Já tens uma recompensa reservada. Utiliza-a antes de criar outro resgate.", 409, { requestId });
    ...
    if (error) {
      console.error("[LOYALTY_REDEEM] rpc_failed", { requestId, code: error.code, message: error.message, details: error.details, hint: error.hint });
    ...
    if (!result?.redemption_id) return errorResponse("Não foi possível criar o resgate.", 503, { requestId });
