...
export default function MyBookingsPage() {
  ...
  async function loadAvailability(item: Appointment, date: string) {
    if (!item || !date) return;
    setAvailabilityLoading(true);
    try {
      const response = await fetch(`/api/customer-portal/appointments/${item.id}?date=${encodeURIComponent(date)}`, { cache: "no-store" });
      const data = await response.json().catch(() => ({}));
  ...
  function openReschedule(item: Appointment) {
    ...
    setAvailability(null);
    void loadAvailability(item, initialDate);
  }
