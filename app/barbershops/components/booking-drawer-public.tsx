...
function isClosedDay(
  shop: BookingDrawerProps['shop'],
  day: BookingDayOption,
): boolean {
...
export function BookingDrawerPublic({
  shop,
  isOpen,
  onClose,
  onSuccess,
    ...
    fetch(
      ...
      .then((data) => {
        if (!controller.signal.aborted) {
          ...
          if (
            selectedServiceId &&
            data.services?.some((item) => item.id === selectedServiceId)
          )
            setServiceId(selectedServiceId);
        }
      })
  ...
  useEffect(() => {
    if (!isOpen || !shop || !professional || !serviceId || closed) {
      setSlots([]);
      return;
    }
    controllerRef.current?.abort();
        ...
        <header className="shrink-0 border-b border-white/10 px-4 py-4 sm:px-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="flex size-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
            >
          ...
          {error ? (
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid gap-2">
                    {professionals.map((item) => (
                      <button
                        ...
                        className={`flex min-h-16 w-full items-center justify-between rounded-2xl border p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${professional?.id === item.id ? 'border-emerald-400/40 bg-emerald-400/[0.08]' : 'border-white/10 bg-white/[0.025]'}`}
                      >
                        <span>
              ...
              <section aria-labelledby="booking-day-title">
                ...
                <div className="flex snap-x gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {days.map((day, index) => {
                    const dayClosed = isClosedDay(shop, day);
                    return (
                      <button
                        key={day.dateStr}
                        type="button"
                        aria-pressed={dayIndex === index}
                        aria-disabled={dayClosed}
                        onClick={() => setDayIndex(index)}
                        className={`min-w-[68px] rounded-2xl border px-2 py-3 text-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${dayIndex === index ? 'border-emerald-400/40 bg-emerald-400/[0.08]' : 'border-white/10 bg-white/[0.025]'} ${dayClosed ? 'opacity-55' : ''}`}
                      >
                ...
                {closed && (
                  <p className="mt-3 rounded-2xl border border-amber-400/20 bg-amber-400/[0.05] p-3 text-xs text-amber-100">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (closed)
                    return toast.error(
                      'Escolhe um dia em que a barbearia esteja aberta.',
                    );
                  if (!service) return toast.error('Escolhe o serviço.');
                  setStep(2);
                }}
                disabled={loadingPeople || !professionals.length || !service}
                className="min-h-11 w-full rounded-xl bg-zinc-100 text-zinc-950 hover:bg-white"
              >
                Continuar{' '}
            ...
            <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {slots.map((slot) => (
                      <button
                        ...
                        className={`min-h-11 rounded-xl border text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${selectedSlot === slot ? 'border-emerald-400/40 bg-emerald-300 text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                      >
                        {slot}
                      </button>
                    ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!selectedSlot)
                      return toast.error('Escolhe um horário.');
                    setStep(3);
                  }}
            ...
            >
              ...
              <div className="space-y-3">
                <label
                  className="grid gap-1.5 text-xs text-zinc-500"
                ...
                >
                  Data de nascimento
                  <span className="text-[11px] text-zinc-600">DD/MM/AAAA</span>
                  <input
              ...
              <p className="text-xs leading-5 text-zinc-600">
                Quando o email e o telefone coincidirem com um cliente existente
                desta barbearia, a marcação é associada automaticamente ao
                perfil existente.
              </p>
              <div className="grid grid-cols-2 gap-2">
                <Button
