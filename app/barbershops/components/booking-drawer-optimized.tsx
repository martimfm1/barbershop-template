...
export function BookingDrawerOptimized({
  shop,
  isOpen,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    ...
    const apply = (data: MarketplaceBookingResponse) => {
      ...
      setSelectedSlot((current) =>
        preserveSlot && current && data.availableSlots?.includes(current)
          ? current
          : null,
      );
    ...
    fetch(`/api/shops/${shop.id}/booking-data?${params.toString()}`, {
      ...
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(
            payload?.error || 'Falha ao carregar a disponibilidade.',
          );
  ...
  const submit = async () => {
    ...
    if (!name.trim() || !phone.trim() || !email.trim() || !birthDate)
        ...
        <DrawerHeader className="shrink-0 border-b border-white/10 px-4 py-3 text-left sm:px-5 sm:py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <DrawerDescription className="mt-1 text-[11px] text-zinc-400">
                {error
                  ? 'Não foi possível concluir a reserva.'
                  : step === 1
                    ? 'Serviço, dia e horário.'
                    : step === 2
                      ? 'Escolhe o barbeiro.'
                      : step === 3
            ...
            {!error && step < 4 && (
              ...
              >
                {[1, 2, 3].map((item) => (
                  <span
                    key={item}
                    className={`flex size-6 items-center justify-center rounded-full text-[10px] font-bold ${step >= item ? 'bg-white text-zinc-950' : 'border border-white/10 bg-white/5 text-zinc-500'}`}
                  >
                    {item}
                  </span>
                ))}
          ...
          {error ? (
            ...
            <div className="space-y-5">
                ...
                <div className="flex snap-x gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {services.map((item) => (
                    <button
                      ...
                      className={`min-w-[148px] shrink-0 snap-start touch-manipulation rounded-xl border p-3 text-left transition ${serviceId === item.id ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
                    >
                      <span className="block truncate text-sm font-bold">
                        {item.name}
                      </span>
                      <span
                        className={`mt-2 block text-xs ${serviceId === item.id ? 'text-zinc-700' : 'text-zinc-400'}`}
                      >
              ...
              <section>
                ...
                <div className="flex snap-x gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {days.map((day, index) => {
                    const closed = shopClosedDays.has(
                    ...
                    return (
                      <button
                        ...
                        className={`min-w-[62px] shrink-0 snap-start touch-manipulation rounded-xl border px-2.5 py-2.5 text-center ${closed ? (index === dayIndex ? 'border-amber-500/40 bg-amber-500/10 text-amber-200' : 'border-amber-500/20 bg-amber-500/[0.04] text-zinc-400') : index === dayIndex ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
                      >
                        ...
                        {closed && (
                          <span className="mt-0.5 block text-[8px] font-semibold uppercase text-amber-400">
                            Folga
                          </span>
                        )}
                      </button>
                ...
                {closedDay && (
                  <div className="mt-3 flex gap-3 rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-3">
                ...
                {blocked.length > 0 && !closedDay && (
                  <div className="mt-3 space-y-2">
                    {blocked.map((block) => (
                      <div
                        key={block.id}
                        className="flex gap-3 rounded-xl border border-orange-500/20 bg-orange-500/[0.06] p-3"
                      >
                    ...
                    {loading ? (
                      ...
                      <>
                        ...
                        <p className="mt-1 text-xs text-zinc-500">
                          Os horários ocupados ou bloqueados não aparecem.
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {slots.map((slot) => (
                      <button
                        ...
                        className={`min-h-11 touch-manipulation rounded-xl border text-sm font-bold active:scale-[0.98] ${selectedSlot === slot ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                      >
                        {slot}
                      </button>
                    ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">
                <UserCheck className="size-3.5" /> Profissional
              </div>
              <button
                type="button"
                onClick={() => setProfessional('any')}
                className={`flex w-full touch-manipulation items-center justify-between rounded-xl border p-4 text-left ${professional === 'any' ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
              >
                <div>
                  <div className="text-sm font-bold">Sem preferência</div>
                  <div
                    className={`text-xs ${professional === 'any' ? 'text-zinc-700' : 'text-zinc-500'}`}
                  >
                    Qualquer profissional disponível
                  </div>
                </div>
                {professional === 'any' && <Check className="size-4" />}
              </button>
              {professionals.map((item) => (
                <button
                  ...
                  className={`flex w-full touch-manipulation items-center justify-between rounded-xl border p-4 text-left ${professional !== 'any' && professional.id === item.id ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
                >
                  <div>
                    <div className="text-sm font-bold">{item.name}</div>
                    <div
                      className={`text-xs ${professional !== 'any' && professional.id === item.id ? 'text-zinc-700' : 'text-zinc-500'}`}
                    >
                      {item.role || 'Barbeiro Profissional'}
                    </div>
                  </div>
                  {professional !== 'any' && professional.id === item.id && (
            ...
            <div className="space-y-5">
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-xs">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="max-w-[65%] truncate text-right font-semibold">
                    {service?.name}
                  </span>
                </div>
              ...
              <div className="min-w-0 space-y-3">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">
                  <User className="size-3.5" /> Os teus dados
                </div>
                <input
                  value={name}
                  ...
                  className="box-border h-12 w-full min-w-0 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20"
                />
                <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[10px] leading-4 text-zinc-600">
                      Podes usar o mesmo email em várias marcações.
                    </p>
                  </div>
                </div>
                <div className="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  <label
                    htmlFor="booking-birth-date"
                    className="mb-1.5 block text-xs font-semibold text-zinc-200"
                  >
                  ...
                  <input
                    ...
                    autoComplete="bday"
                    required
                    className="box-border block h-12 w-full min-w-0 max-w-full appearance-none rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20"
                  />
                  <p className="mt-1.5 text-[11px] leading-4 text-zinc-500">
                    Usamos a tua data de nascimento apenas para questões de
                    estatística.
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              <div className="flex size-16 items-center justify-center rounded-full border border-emerald-500/20 bg-emerald-500/10 text-emerald-400">
                <CheckCircle2 className="size-8" />
              </div>
              <div>
                <h3 className="text-xl font-bold">Tudo pronto e reservado!</h3>
                <p className="mt-1 text-xs text-zinc-500">
                  O teu horário foi garantido.
                </p>
              </div>
              <div className="flex w-full items-start gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-4 text-left text-xs">
        ...
        <DrawerFooter className="shrink-0 border-t border-white/10 px-3 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:px-4">
          <div className="flex items-center gap-2 sm:justify-between sm:gap-3">
            {error ? (
              ...
              <Button
                onClick={onClose}
                className="min-h-11 w-full bg-white font-bold text-zinc-950"
              >
                Concluir
              </Button>
            ) : (
              <>
                ...
                {step < 3 ? (
                  <Button
                    disabled={
                      step === 1
                        ? !stepOneReady || loading
                        : !stepTwoReady || loading
                    }
                    onClick={() => setStep((step + 1) as 2 | 3)}
                    className="min-h-11 flex-[1.4] bg-white font-bold text-zinc-950"
                  >
                    Próximo
