...
export function BookingDrawerStable({
  shop,
  isOpen,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    ...
    fetch(`/api/shops/${shop.id}/booking-data?${params.toString()}`, {
      ...
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(
            payload?.error || 'Não foi possível carregar a disponibilidade.',
          );
      ...
      .then((data) => {
        ...
        if (
          serviceId &&
          !(data.services ?? []).some((item) => item.id === serviceId)
        )
        ...
        setSelectedSlot((current) =>
          current && data.availableSlots?.includes(current) ? current : null,
        );
      })
      .catch((error) => {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok || !payload.success)
        throw new Error(
          payload.error || 'Não foi possível concluir a marcação.',
        );
      setStep(4);
      cacheRef.current.clear();
      ...
      <DrawerContent className="mx-auto flex h-[100dvh] max-h-[100dvh] w-full max-w-lg flex-col overflow-hidden rounded-none border-white/10 bg-zinc-950 text-zinc-100 sm:h-auto sm:max-h-[90dvh] sm:rounded-t-3xl">
        <div className="flex min-h-0 flex-1 flex-col">
          <header className="shrink-0 border-b border-white/10 px-4 py-3 sm:px-5 sm:py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <h2 className="mt-1 truncate text-base font-semibold sm:text-xl">
                  {step === 4 ? 'Reserva confirmada' : shop.name}
                </h2>
                <p className="mt-1 text-xs text-zinc-400">
                  {step === 1
                    ? 'Escolhe serviço, dia e hora.'
                    : step === 2
                      ? 'Escolhe um barbeiro ou deixa sem preferência.'
                      : step === 3
                        ? 'Confirma os teus dados.'
                        : 'O teu horário está reservado.'}
                </p>
              </div>
              <button
                type="button"
                ...
                className="flex size-11 shrink-0 touch-manipulation items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] text-zinc-300 hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
              >
                <X className="size-4" />
              </button>
            </div>
            {step < 4 && (
              ...
              >
                {[1, 2, 3].map((item) => (
                  <div
                    key={item}
                    className={`h-1.5 rounded-full ${step >= item ? 'bg-emerald-300' : 'bg-white/10'}`}
                  />
                ))}
              </div>
            )}
          ...
          <main className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 pb-6 sm:px-5">
            {loadError && step < 4 ? (
              <div className="mb-4 flex gap-3 rounded-2xl border border-red-500/20 bg-red-500/[0.06] p-4">
                <XCircle className="mt-0.5 size-5 shrink-0 text-red-400" />
                <div>
                  <p className="text-sm font-semibold text-red-200">
                    Não foi possível carregar o booking
                  </p>
            ...
            {step === 1 && (
              <div className="space-y-5">
                <section>
                  <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">
                    ...
                    {services.length ? (
                      services.map((item) => (
                        <button
                          ...
                          className={`min-w-[155px] shrink-0 rounded-2xl border p-3 text-left touch-manipulation ${serviceId === item.id ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                        >
                          <span className="block truncate text-sm font-semibold">
                            {item.name}
                          </span>
                          <span
                            className={`mt-2 block text-xs ${serviceId === item.id ? 'text-zinc-700' : 'text-zinc-500'}`}
                          >
                      ...
                      <div className="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-zinc-500">
                        {loading
                          ? 'A carregar serviços…'
                ...
                <section>
                    ...
                    {days.map((day, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={`min-w-[64px] shrink-0 rounded-2xl border px-2 py-3 text-center touch-manipulation ${index === dayIndex ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
                        >
                          ...
                          {closedDay && (
                            <span className="mt-1 block text-[8px] font-semibold uppercase text-amber-300">
                              Folga
                            </span>
                          )}
                        </button>
                  ...
                  {closedDay && (
                    <div className="mt-3 flex gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-3">
                ...
                <section>
                  ...
                  {slots.length ? (
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {slots.map((slot) => (
                        <button
                          ...
                          className={`min-h-12 rounded-2xl border text-sm font-semibold touch-manipulation ${selectedSlot === slot ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200 hover:bg-white/[0.06]'}`}
                        >
                          {slot}
                        </button>
                      ))}
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center text-sm text-zinc-500">
                      {loading
                        ? 'A verificar horários…'
                        : closed
                          ? 'Não existem horários disponíveis para este dia.'
                          : 'Seleciona um serviço para ver os horários.'}
                    </div>
                  )}
                </section>
              </div>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <section>
                  <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">
                    <UserCheck className="size-3.5" /> Barbeiro
                  </div>
                  <button
                    type="button"
                    onClick={() => setProfessional('any')}
                    className={`w-full rounded-2xl border p-4 text-left touch-manipulation ${professional === 'any' ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold">Sem preferência</p>
                        <p
                          className={`mt-1 text-xs ${professional === 'any' ? 'text-zinc-700' : 'text-zinc-500'}`}
                        >
                    ...
                    {professionals.length ? (
                      professionals.map((item) => (
                        <button
                          ...
                          className={`flex w-full items-center justify-between gap-3 rounded-2xl border p-4 text-left touch-manipulation ${professional !== 'any' && professional.id === item.id ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                        >
                          <div>
                            <p className="text-sm font-semibold">{item.name}</p>
                            <p
                              className={`mt-1 text-xs ${professional !== 'any' && professional.id === item.id ? 'text-zinc-700' : 'text-zinc-500'}`}
                            >
                      ...
                      <div className="rounded-2xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">
                        Não existem perfis de barbeiro associados a esta
                        barbearia.
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-5">
                ...
                <section className="space-y-3">
                  <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">
                    <User className="size-3.5" /> Dados do cliente
                  </div>
                  <label className="block">
                    ...
                    <input
                      ...
                      className="h-12 w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-emerald-400/50 focus:ring-4 focus:ring-emerald-400/10"
                    />
                  </label>
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      aria-describedby="booking-birth-hint"
                      className="h-12 w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-emerald-400/50 focus:ring-4 focus:ring-emerald-400/10"
                    />
                    <p
                      id="booking-birth-hint"
                      className="mt-1.5 text-[11px] leading-4 text-zinc-500"
                    >
                      Introduz no formato DD/MM/AAAA.
                    </p>
                    {birthDateDisplay.length === 10 && !birthDateIso ? (
            ...
            {step === 4 && (
              <div className="flex flex-col items-center gap-5 py-10 text-center">
                ...
                <div>
                  <h3 className="text-xl font-semibold">Tudo reservado.</h3>
                  <p className="mt-1 text-sm text-zinc-500">
                    A confirmação foi enviada para o teu email.
                  </p>
                </div>
                <div className="flex w-full items-start gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left text-sm">
                  <Mail className="mt-0.5 size-4 shrink-0 text-emerald-300" />
                  <span className="break-all text-zinc-400">{email}</span>
                </div>
              </div>
            )}
          </main>
          ...
          <footer className="shrink-0 border-t border-white/10 px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:px-5">
            <div className="flex items-center gap-2">
              {step === 4 ? (
                <Button
                  onClick={onClose}
                  className="min-h-12 w-full bg-white font-semibold text-zinc-950"
                >
                  Concluir
                </Button>
              ) : (
                <>
                  ...
                  {step === 1 ? (
                    <Button
                      disabled={!service || !selectedSlot || loading || closed}
                      onClick={() => setStep(2)}
                      className="min-h-12 flex-[1.4] bg-white font-semibold text-zinc-950"
                    >
                      Continuar
                    ...
                    >
                      ...
                      Confirmar · €
                      {service ? Number(service.price ?? 0).toFixed(2) : '0.00'}
                    </Button>
                  )}
                </>
              )}
