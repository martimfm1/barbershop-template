...
export function MapPreview({ shops, view, onSelectShop }: MapPreviewProps) {
  return (
    <motion.aside
      whileHover={{ y: -2 }}
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-zinc-300 backdrop-blur-xl">
          {view === "grid" ? "Grid synced" : "Map active"}
        </span>
      </div>

      <div className="relative mt-5 h-135 overflow-hidden rounded-3xl border border-white/10">
        <MapInner shops={shops} onSelectShop={onSelectShop} />
        ...
        <div className="pointer-events-none absolute bottom-4 left-4 right-4 z-[1000] rounded-2xl border border-white/10 bg-zinc-950/80 p-4 backdrop-blur-md">
          <div className="flex items-center justify-between gap-3 text-sm">
            <div>
              <div className="font-medium text-zinc-50">Live Partners</div>
              <div className="mt-0.5 text-xs text-zinc-400">
                {shops.length} barbershops visible
              </div>
            </div>
