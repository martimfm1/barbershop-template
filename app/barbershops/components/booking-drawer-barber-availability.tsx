...
export function BookingDrawerBarberAvailability({
  shop,
  isOpen,
  onClose,
  onSuccess,
  ...
  const shopClosedDays = useMemo(() => {
    ...
    for (const value of values) {
      ...
      else if (DAY_INDEX[key] !== undefined) set.add(DAY_INDEX[key]);
    }
    return set;
  }, [shop]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/shops/${shop.id}/booking-data?${params}`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok)
          throw new Error(
            data?.error || 'Não foi possível carregar a disponibilidade.',
          );
  ...
  function chooseProfessional(item: MarketplaceProfessional | 'any') {
  ...
  if (!shop) return null;
  const selectedProfessionalAvailable =
    professional === 'any'
      ? true
      : Boolean(
          professionalAvailability[professional.id]?.includes(
            selectedSlot ?? '',
          ),
        );
          ...
          <header className="shrink-0 border-b border-white/10 px-4 py-3 sm:px-5 sm:py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-zinc-400">
                  {step === 1
                    ...
                    : step === 2
                      ? 'Vê quem está disponível para o horário.'
                      : step === 3
              ...
              <button
                ...
                className="flex size-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] text-zinc-300 hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
              >
                <X className="size-4" />
              </button>
            </div>
          ...
          <main className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 pb-6 sm:px-5">
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <section>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {days.map((day, index) => (
                      <button
                        ...
                        className={`min-w-[64px] shrink-0 rounded-2xl border px-2 py-3 text-center touch-manipulation ${index === dayIndex ? 'border-white/50 bg-white text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200'}`}
                      >
                  ...
                  {closedDay && (
                    <div className="mt-3 rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-4 text-sm text-amber-100">
                      <div className="flex gap-3">
                        <AlertCircle className="mt-0.5 size-4 shrink-0 text-amber-300" />
                        <span>A barbearia não aceita marcações neste dia.</span>
                      </div>
                    </div>
                  )}
                </section>
                <section>
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">
                      <Clock3 className="size-3.5" /> Horário
                    </div>
                    {loading && (
                    ...
                    {slots.length ? (
                      slots.map((slot) => (
                        <button
                          ...
                          className={`min-h-11 rounded-xl border text-sm font-semibold transition ${selectedSlot === slot ? 'border-emerald-300 bg-emerald-300 text-zinc-950' : 'border-white/10 bg-white/[0.03] text-zinc-200 hover:border-white/20'}`}
                        >
                          {slot}
                        </button>
                      ))
                    ) : (
                      <div className="col-span-full rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center text-sm text-zinc-500">
                        {loading
                          ? 'A procurar horários…'
                ...
                <Button
                  ...
                  disabled={!selectedSlot || closed || loading}
                  onClick={() => setStep(2)}
                >
                  Escolher barbeiro
                </Button>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-white">
                    {currentDay.fullDateFormatted} · {selectedSlot}
                  </p>
                ...
                <button
                  ...
                  className={`w-full rounded-2xl border p-4 text-left ${professional === 'any' ? 'border-emerald-300/40 bg-emerald-300/[0.08]' : 'border-white/10 bg-white/[0.025]'}`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-500">
                          Escolhe automaticamente um profissional disponível.
                        </p>
                      </div>
                    </div>
                ...
                {professionals.map((item) => {
                  const available = Boolean(
                    professionalAvailability[item.id]?.includes(
                      selectedSlot ?? '',
                    ),
                  );
                  const selected =
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={!available}
                      onClick={() => chooseProfessional(item)}
                      className={`w-full rounded-2xl border p-4 text-left transition ${selected ? 'border-emerald-300/50 bg-emerald-300/[0.08]' : available ? 'border-white/10 bg-white/[0.025] hover:border-white/20' : 'cursor-not-allowed border-white/5 bg-white/[0.015] opacity-60'}`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                          <div className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-sm font-semibold">
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${available ? 'border border-emerald-400/20 bg-emerald-400/10 text-emerald-200' : 'border border-red-400/15 bg-red-400/[0.06] text-red-300'}`}
                        >
                ...
                {!professionals.length && (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-4 text-sm text-amber-100">
                    Não existem perfis de barbeiro configurados para esta
                    barbearia.
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <Button
                    type="button"
                    variant="outline"
                    className="min-h-12 border-white/10 bg-transparent text-white"
                    onClick={() => setStep(1)}
                  >
                  ...
                  <Button
                    ...
                    disabled={
                      !selectedSlot ||
                      (professional !== 'any' && !selectedProfessionalAvailable)
                    }
                    onClick={() => setStep(3)}
                  >
                    Continuar
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
                  ...
                  <p className="mt-1 text-sm font-semibold text-white">
                    {service?.name} · {currentDay.fullDateFormatted} ·{' '}
                    {selectedSlot}
                  </p>
                  <p className="mt-1 text-xs text-zinc-500">
                    {professional === 'any'
            ...
            {step === 4 && (
              <div className="flex flex-col items-center gap-5 py-10 text-center">
                <div className="flex size-16 items-center justify-center rounded-full border border-emerald-400/25 bg-emerald-400/10 text-emerald-300">
                  <Check className="size-7" />
                </div>
                <div>
                  <h3 className="text-xl font-semibold">Reserva confirmada</h3>
                  <p className="mt-2 text-sm text-zinc-400">
                    Receberás a confirmação no email {email}.
                  </p>
                </div>
                <div className="w-full rounded-2xl border border-white/10 bg-white/[0.025] p-4 text-left">
