...
export async function generateMetadata({
  params,
}: LoyaltyPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/barbershops/${encodeURIComponent(profile.slug)}/loyalty`,
    },
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-4 py-10 text-white sm:px-6 sm:py-14">
      <div className="mx-auto w-full max-w-4xl">
        <Link
          href={`/barbershops/${encodeURIComponent(profile.slug)}`}
        ...
        <header className="mt-8 max-w-2xl">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-5xl">
            Fica perto da tua próxima recompensa.
          </h1>
          <p className="mt-4 text-sm leading-7 text-zinc-400 sm:text-base">
            Entra com o teu email e um código de confirmação. Sem password e sem
