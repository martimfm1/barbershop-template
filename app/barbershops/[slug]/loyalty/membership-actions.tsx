...
export default function LoyaltyMembershipActions({ slug }: Props) {
  ...
  return authenticated ? (
    <div className="flex justify-end">
      <button type="button" onClick={() => void leave()} disabled={busy} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-400/20 bg-red-400/[0.04] px-4 text-xs font-medium text-red-200 transition hover:bg-red-400/[0.08] disabled:opacity-50">
    ...
    <div className="flex justify-end">
      <button type="button" onClick={enter} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/[0.05] px-4 text-xs font-medium text-emerald-200 transition hover:bg-emerald-400/[0.10]">
