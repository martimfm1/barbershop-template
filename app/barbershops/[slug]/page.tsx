...
const PUBLIC_ORIGIN = (
  process.env.NEXT_PUBLIC_APP_URL?.trim() || 'https://barbers.silentra.me'
).replace(/\/$/, '');
...
export async function generateMetadata({ params }: BarbershopPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: 'website', title, description, url: absoluteUrl(canonicalPath), siteName: 'Silentra for Barbers', ...(image ? { images: [{ url: image, alt: profile.name }] } : {}), locale: 'pt_PT' },
...
async function getInitialPublicDetails(profile: PublicProfileRecord): Promise<BarbershopPublicDetails> {
  ...
  const [{ data: services }, { data: reviews }, { data: barbershop }, { data: shop }] = await Promise.all([
    ...
    database.from('barbershops').select('opening_time, closing_time, lunch_start, lunch_end, closed_days').eq('id', barbershopId).maybeSingle(),
    database.from('shops').select('amenities').eq('barbershop_id', barbershopId).maybeSingle(),
  ]);

  const reviewItems = reviews ?? [];
  const ratingAverage = reviewItems.length ? Number((reviewItems.reduce((total, review) => total + Number(review.rating || 0), 0) / reviewItems.length).toFixed(1)) : 0;
  return {
    ...profile,
    amenities: normalizeBarbershopAmenities(shop?.amenities),
    services: (services ?? []).map((service) => ({ ...service, popular: profile.plan !== 'free' && service.popular === true })),
    reviews: reviewItems,
...
async function getPublicLoyaltyEnabled(profile: PublicProfileRecord): Promise<boolean> {
  ...
  const database = createAdminClient();
  const { data, error } = await database.from('loyalty_settings').select('enabled').eq('barbershop_id', profile.barbershop_id ?? profile.id).maybeSingle();
  if (error) {
    console.error('[PUBLIC_BARBERSHOP]', { event: 'loyalty_settings_lookup_failed', slug: profile.slug, code: error.code ?? 'UNKNOWN' });
    return false;
  }
...
export default async function BarbershopPage({ params }: BarbershopPageProps) {
  ...
  if (UUID_PATTERN.test(slug)) {
    ...
    if (!profile) { logPublicProfile('warn', 'id_resolution_failed', slug); notFound(); }
  ...
  if (!isValidPublicProfileSlug(slug)) { logPublicProfile('warn', 'invalid_slug', slug); notFound(); }
