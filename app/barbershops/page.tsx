...
export default function BarbershopsDirectoryPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 antialiased">
      <SiteNavbar />
      <main className="mx-auto w-full max-w-7xl px-3 pb-16 pt-[76px] sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
        <header className="relative overflow-hidden rounded-[1.25rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.11),transparent_36%),rgba(24,24,27,0.72)] p-4 shadow-[0_24px_70px_rgba(0,0,0,0.24)] sm:rounded-[2rem] sm:p-8 lg:p-10">
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-[2rem] font-semibold leading-[1.02] tracking-[-0.055em] sm:mt-5 sm:text-5xl">Encontra a barbearia certa e reserva sem complicações.</h1>
          ...
          <div className="mt-5 grid gap-2.5 sm:mt-7 sm:gap-3 lg:max-w-4xl">
            ...
            <div className="sm:hidden"><MobileSearchFilterBar query={query} setQuery={setQuery} activeDate={activeDate} setActiveDate={setActiveDate} activeFilter={activeFilter} setActiveFilter={activeFilter === "Near Me" && !userLocation ? "All" : setActiveFilter} view={view} setView={setView} userLocation={userLocation} setUserLocation={setUserLocation} /></div>
            <div className="sm:hidden"><LocationRequest value={userLocation} onChange={setUserLocation} autoRequest /></div>
          </div>

          {availableTags.length > 0 && (
            <div className="mt-2.5 rounded-2xl border border-white/10 bg-black/20 p-2.5 sm:mt-4 sm:p-4">
              <div className="mb-2 flex items-center gap-2 px-0.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500"><Tag className="size-3.5" />Categorias e estilos</div>
              ...
              {activeTag && <div className="mt-2 flex items-center justify-between gap-2 border-t border-white/5 pt-2 text-[11px] text-zinc-500"><span>Filtro: <strong className="text-zinc-300">{activeTag}</strong></span><button type="button" onClick={() => setActiveTag(null)} className="inline-flex min-h-8 items-center gap-1 rounded-lg px-2 text-zinc-400 hover:bg-white/5 hover:text-zinc-200"><X className="size-3.5" />Limpar</button></div>}
            </div>
          )}
        </header>
        ...
        <section className="mt-5 sm:mt-8">
          <div className="mb-3 flex flex-col gap-3 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2 sm:hidden"><button type="button" onClick={() => setView("grid")} aria-pressed={view === "grid"} className={`min-h-10 flex-1 rounded-xl border px-3 text-xs font-semibold ${view === "grid" ? "border-white/20 bg-white text-zinc-950" : "border-white/10 bg-white/[0.03] text-zinc-400"}`}>Lista</button><button type="button" onClick={() => setView("map")} aria-pressed={view === "map"} className={`min-h-10 flex-1 rounded-xl border px-3 text-xs font-semibold ${view === "map" ? "border-white/20 bg-white text-zinc-950" : "border-white/10 bg-white/[0.03] text-zinc-400"}`}>Mapa</button></div>
          </div>

          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div className="min-w-0">
              {isLoading ? <div className="flex min-h-64 items-center justify-center rounded-3xl border border-white/10 bg-zinc-900/50" aria-live="polite"><div className="flex flex-col items-center gap-3 text-sm text-zinc-500"><Loader2 className="size-6 animate-spin" /><span>A encontrar barbearias...</span></div></div> : visibleShops.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] p-8 text-center sm:p-12"><p className="text-base font-semibold text-zinc-200">Não encontrámos resultados.</p><p className="mt-2 text-sm leading-6 text-zinc-500">Tenta remover um filtro, procurar outra zona ou ativar a localização.</p></div> : view === "map" ? <div className="h-[min(68svh,560px)] overflow-hidden rounded-3xl border border-white/10"><MapPreview shops={visibleShops} view={view} onSelectShop={handleNavigateToShop} userLocation={userLocation} /></div> : <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">{visibleShops.map((shop) => <ShopCard key={shop.id} shop={shop} onNavigate={() => handleNavigateToShop(shop)} onOpenBooking={() => setBookingShop(shop)} />)}</div>}
            </div>
