    ...
    <div className="silentra-page-shell min-h-[100svh] overflow-x-clip text-zinc-50 antialiased">
      ...
      <main className="relative z-10 mx-auto w-full max-w-7xl px-3 pb-[max(4rem,env(safe-area-inset-bottom))] pt-[calc(5rem+env(safe-area-inset-top))] sm:px-6 sm:pb-16 sm:pt-28 lg:px-8 lg:pt-32">
        <header className="silentra-section-block overflow-hidden rounded-[1.5rem] p-4 sm:rounded-[2rem] sm:p-8 lg:p-10">
          <div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-emerald-300/30 to-transparent" aria-hidden="true" />
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-[13px] leading-5 text-zinc-400 sm:mt-4 sm:text-base sm:leading-6">
              Compara localização, disponibilidade, etiquetas e avaliações. A localização é opcional e podes alterar os critérios quando quiseres.
            </p>
          </div>
          ...
          {availableTags.length > 0 && (
            <div className="mt-2 rounded-2xl border border-white/10 bg-black/15 p-2 sm:mt-4 sm:p-4">
              <div className="mb-2 flex items-center gap-2 px-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500"><Tag className="size-3.5" aria-hidden="true" />Categorias e estilos</div>
        ...
        <section className="mt-5 sm:mt-8">
          <div className="mb-3 flex flex-col gap-2.5 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0"><p className="silentra-eyebrow mb-1">Resultados</p><p className="text-sm text-zinc-400" aria-live="polite">{isLoading ? "A procurar..." : `${visibleShops.length} ${visibleShops.length === 1 ? "barbearia encontrada" : "barbearias encontradas"}`}</p></div>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div className="min-w-0">
              {isLoading ? (
                <div className="silentra-section-block flex min-h-64 items-center justify-center rounded-3xl" aria-live="polite"><div className="flex flex-col items-center gap-3 text-sm text-zinc-500"><Loader2 className="size-6 animate-spin text-emerald-300" aria-hidden="true" /><span>A encontrar barbearias...</span></div></div>
              ) : visibleShops.length === 0 ? (
                <div className="silentra-section-block rounded-3xl border-dashed p-7 text-center sm:p-12"><div className="mx-auto flex size-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] text-zinc-400"><Search className="size-5" aria-hidden="true" /></div><p className="mt-4 text-base font-semibold text-zinc-200">Não encontrámos resultados.</p><p className="mt-2 text-sm leading-6 text-zinc-500">Tenta remover um filtro, procurar outra zona ou ativar a localização.</p></div>
              ) : view === "map" ? (
                <div className="h-[calc(100svh-15rem)] min-h-[28rem] max-h-[38rem] overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/60 shadow-2xl"><MapPreview shops={visibleShops} view={view} onSelectShop={handleNavigateToShop} userLocation={userLocation} /></div>
              ) : (
                <div className="grid gap-3 md:grid-cols-2 sm:gap-4">{visibleShops.map((shop) => <ShopCard key={shop.id} shop={shop} onNavigate={() => handleNavigateToShop(shop)} onOpenBooking={() => setBookingShop(shop)} />)}</div>
              )}
            </div>
            <aside className="hidden lg:block"><div className="silentra-section-block sticky top-24 overflow-hidden rounded-3xl"><div className="border-b border-white/8 px-4 py-3"><div className="flex items-center gap-2 text-xs font-semibold text-zinc-200"><MapPin className="size-4 text-emerald-300" aria-hidden="true" />Explora no mapa</div><p className="mt-1 text-[11px] text-zinc-500">Seleciona uma barbearia para abrir a página completa.</p></div><div className="h-[28rem]"><MapPreview shops={visibleShops} view={view} onSelectShop={handleNavigateToShop} userLocation={userLocation} /></div></div></aside>
          </div>
        </section>
      </main>
