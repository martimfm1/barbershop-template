...
function applyTokens(value: string, vars: { nome: string; barbearia: string; bookingUrl: string }) {
  return value
...
async function getCampaign(id: string): Promise<Campaign> {
  ...
  const { data, error } = await admin
    ...
    .select('id,barbershop_id,name,channel,subject,body,trigger_type,interval_value,interval_unit,event_name,birthday_offset_days,birthday_last_run_date,next_run_at')
    .eq('id', id)
    .maybeSingle();
...
async function queueRecipients(campaignId: string, runKey: string, targetClientId?: string | null) {
  const admin = createAdminClient();
  ...
  const { data: clients, error, count } = await query.limit(targetClientId ? 1 : 5000);
  if (error) throw error;

  if (!clients?.length) {
    const next =
      campaign.trigger_type === 'interval' && campaign.interval_value && campaign.interval_unit
...
export async function processQueuedCampaignRecipients(limit = BATCH_SIZE) {
  ...
  for (const recipient of recipients) {
    ...
    try {
      ...
      const [{ data: client }, { data: shop }] = await Promise.all([
        admin.from('users').select('name_complete').eq('id', recipient.client_id).eq('barbershop_id', campaign.barbershop_id).maybeSingle(),
      ...
      const subject = applyTokens(campaign.subject?.trim() || `${campaign.name} — ${shopName}`, vars);
      ...
      if (result.success) {
        ...
        await admin.from('marketing_campaign_recipients').update({
          ...
          next_attempt_at: attempts >= MAX_ATTEMPTS ? null : new Date(Date.now() + attempts * 60000).toISOString(),
  ...
  for (const campaignId of campaignIds) {
    const [{ count: queued }, { count: sending }, { count: sentCount }, { count: failedCount }] = await Promise.all([
...
export async function dispatchMarketingEvent(input: { barbershopId: string; eventName: string; clientId?: string | null }) {
  ...
  const admin = createAdminClient();
  const { data: campaigns, error } = await admin.from('marketing_campaigns').select('id').eq('barbershop_id', input.barbershopId).eq('active', true).eq('trigger_type', 'event').eq('event_name', input.eventName).limit(50);
  if (error) throw error;
  let triggered = 0;
  for (const campaign of campaigns ?? []) {
    await queueCampaign(campaign.id, `event:${input.eventName}:${input.clientId}:${crypto.randomUUID()}`, input.clientId);
    triggered++;
  }
  return { triggered };
}
...
export async function processScheduledCampaigns(limit = 25) {
  const admin = createAdminClient();
  const { data: campaigns, error } = await admin.from('marketing_campaigns').select('id').eq('active', true).eq('trigger_type', 'interval').eq('status', 'scheduled').lte('next_run_at', new Date().toISOString()).limit(limit);
  if (error) throw error;
  let processed = 0;
  for (const campaign of campaigns ?? []) {
    await queueCampaign(campaign.id, `scheduled:${campaign.id}:${crypto.randomUUID()}`);
    processed++;
  }
  return { processed };
}
...
export async function processBirthdayCampaigns(limit = 25) {
  ...
  const { data: campaigns, error } = await admin
    .from('marketing_campaigns')
    .select('id,barbershop_id,channel,birthday_offset_days,birthday_last_run_date')
    .eq('active', true)
  ...
  for (const campaign of campaigns ?? []) {
    ...
    const matching = (clients ?? []).filter((client) => String(client.birth_date ?? '').slice(5) === monthDay);
    ...
    for (const client of matching) {
      await queueCampaign(campaign.id, `birthday:${campaign.id}:${today}:${client.id}`, client.id);
    }
    processed++;
  }
