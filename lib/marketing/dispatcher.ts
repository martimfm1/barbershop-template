...
function getLisbonDate(offsetDays = 0) {
  const now = new Date();
...
export async function queueCampaign(
  ...
  const { data: clients, error: clientsError, count } = await query.limit(
    targetClientId ? 1 : 5000,
  );
...
export async function processQueuedCampaignRecipients(
  ...
  for (const recipient of recipients) {
    ...
    try {
      ...
      const [{ data: client }, { data: shop }] = await Promise.all([
        admin
          .from('users')
          .select('name_complete,email,num_phone')
      ...
      const body = token(campaign.body, {
        nome: name,
        barbearia: shopName,
        bookingUrl,
      });
      const subject = token(
        ...
        { nome: name, barbearia: shopName, bookingUrl },
      );
  ...
  for (const campaignId of campaignIds) {
    const [
      { count: queued },
      { count: sending },
      { count: sentForCampaign },
      { count: failedForCampaign },
    ] = await Promise.all([
      admin
        .from('marketing_campaign_recipients')
    ...
    const update: Record<string, unknown> = {
      sent_count: sentForCampaign ?? 0,
      failed_count: failedForCampaign ?? 0,
      updated_at: new Date().toISOString(),
    };
    if (complete) {
...
export async function dispatchMarketingEvent(input: {
  ...
  if (!input.clientId) {
    console.warn('[MARKETING_EVENT_SKIPPED_NO_CLIENT]', {
  ...
  for (const campaign of campaigns ?? []) {
    await queueCampaign(
      campaign.id,
      `event:${input.eventName}:${input.clientId}:${Date.now()}:${crypto.randomUUID()}`,
      input.clientId,
    );
    queued++;
  }
...
export async function processScheduledCampaigns(limit = 25) {
  ...
  for (const campaign of campaigns ?? []) {
    await queueCampaign(
      campaign.id,
      `scheduled:${campaign.id}:${Date.now()}:${crypto.randomUUID()}`,
    );
    processed++;
  }
  return { processed };
...
export async function processBirthdayCampaigns(limit = 25) {
  const admin = createAdminClient();
  const today = getLisbonDate();
  const { data: campaigns, error } = await admin
    .from('marketing_campaigns')
    .select(
      'id,barbershop_id,trigger_type,birthday_offset_days,birthday_last_run_date,active,status',
    )
    .eq('active', true)
  ...
  for (const campaign of campaigns ?? []) {
    ...
    const { data: claimed } = await admin
      ...
      .eq('trigger_type', 'birthday')
      .or(
        `birthday_last_run_date.is.null,birthday_last_run_date.neq.${today}`,
      )
      .select('id')
      .maybeSingle();
    ...
    const targetDate = getLisbonDate(-(campaign.birthday_offset_days ?? 0));
    ...
    const matching = (clients ?? []).filter((client) => {
      const birth = String(client.birth_date ?? '');
      return birth.slice(5, 7) === month && birth.slice(8, 10) === day;
    });
