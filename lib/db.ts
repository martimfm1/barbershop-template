...
function normalizeError(operation: string, error: unknown): Error {
  ...
  const fallback = new Error(`Database operation failed: ${operation}`);
  console.error(`[db] ${operation} failed:`, fallback.message);
  return fallback;
}
...
export async function getRecordById<T>(supabase: SupabaseClientLike, table: string, id: string): Promise<DbResult<T>> {
...
export async function insertRecord<T>(supabase: SupabaseClientLike, table: string, data: Partial<T>): Promise<DbResult<T>> {
...
export async function updateRecord<T>(supabase: SupabaseClientLike, table: string, id: string, data: Partial<T>): Promise<DbResult<T>> {
...
export async function deleteRecord(supabase: SupabaseClientLike, table: string, id: string): Promise<DbResult<null>> {
  const { error } = await supabase.from(table).delete().eq("id", id);

  return toResult<null>(null, error);
}

export async function getUserBarbershopId(supabase: SupabaseClientLike, userId: string) {
  const { data, error } = await supabase.from("users").select("barbershop_id").eq("id", userId).maybeSingle();
