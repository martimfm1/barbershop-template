...
const OPTIONS: LanguageOption[] = [
  { value: "pt", label: "Português", short: "PT", description: "Português (Portugal)" },
...
export function LanguageSwitcher() {
  ...
  useEffect(() => {
    const syncMobileMenuState = () => {
      setMobileMenuOpen(Boolean(document.getElementById("site-navigation-overlay")));
    };
  ...
  const ariaLabel = locale === "pt" ? `Idioma: ${current.label}` : `Language: ${current.label}`;
    ...
    <AnimatePresence initial={false}>
      {!mobileMenuOpen ? (
        ...
        >
          ...
          >
            <Languages className="size-3.5 text-zinc-500 transition-colors group-hover:text-zinc-300" aria-hidden="true" />
            <span className="hidden text-zinc-500 sm:inline">{label}</span>
            <span className="font-semibold tracking-[0.08em] text-zinc-200">{current.short}</span>
            <ChevronDown className={`size-3 text-zinc-500 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} aria-hidden="true" />
          </motion.button>
                    ...
                    return (
                      <button
                        ...
                        className={`flex min-h-12 w-full items-center gap-3 rounded-lg px-3 text-left transition-[background-color,color] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 ${
                          selected ? "bg-white/[0.07] text-white" : "text-zinc-400 hover:bg-white/[0.04] hover:text-zinc-100"
                        }`}
                      >
                        <span className="flex size-7 items-center justify-center rounded-md border border-white/8 bg-white/[0.025] text-[10px] font-semibold tracking-[0.08em] text-zinc-400" aria-hidden="true">
                          {option.short}
                        </span>
                        ...
                        {selected ? <Check className="size-4 shrink-0 text-zinc-200" aria-hidden="true" /> : null}
                      </button>
                    );
                  })}
                </div>
