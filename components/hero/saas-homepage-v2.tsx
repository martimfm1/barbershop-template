...
function SectionHeading({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) {
  return <div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-emerald-300/80">{eyebrow}</p><h2 className="mt-3 text-3xl font-semibold leading-tight tracking-[-0.05em] text-white sm:text-5xl">{title}</h2><p className="mt-4 text-sm leading-6 text-zinc-400 sm:text-base">{text}</p></div>;
}
...
export function SaaSHomepageV2() {
  ...
  return (
    <main className="relative overflow-hidden">
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-16 pt-5 sm:px-6 sm:pb-24 sm:pt-10 lg:px-8 lg:pb-28 lg:pt-16">
        <div className="grid items-center gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16">
          <div>
            ...
            <div className="mt-7 grid gap-3 sm:flex sm:flex-wrap"><Link href="/registo" className="inline-flex min-h-12 items-center justify-center gap-2 bg-white px-5 text-sm font-semibold text-zinc-950 transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">Criar barbearia grátis <ArrowRight className="size-4" /></Link><Link href="/barbershops" className="inline-flex min-h-12 items-center justify-center gap-2 border border-white/10 bg-white/[0.04] px-5 text-sm font-semibold text-zinc-100 transition hover:border-white/20 hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30">Explorar barbearias</Link></div>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-18 sm:px-6 sm:pb-24 lg:grid-cols-2 lg:px-8"><article className="border border-white/10 bg-zinc-900/45 p-5 sm:p-8"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-500">Depois da reserva</p><h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white sm:text-3xl">A relação não acaba no botão “confirmar”.</h2><div className="mt-7 space-y-3">{["O cliente consulta todas as marcações associadas ao mesmo email.", "Pode cancelar dentro da janela definida pela barbearia.", "Pode reagendar para horários que respeitam a operação real."].map((text) => <div key={text} className="flex gap-3 border border-white/8 bg-black/20 p-4 text-sm leading-6 text-zinc-300"><Check className="mt-0.5 size-4 shrink-0 text-emerald-300" />{text}</div>)}</div><Link href="/my-bookings" className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-white hover:text-emerald-200">Gerir uma marcação <ArrowRight className="size-4" /></Link></article><article className="border border-white/10 bg-zinc-900/45 p-5 sm:p-8"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-500">Gestão</p><h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white sm:text-3xl">O plano pertence à barbearia. O acesso pertence às permissões.</h2><div className="mt-7 space-y-3">{["Todos os membros herdam as funcionalidades do plano do tenant.", "Owner, admin e barbeiro podem ter acessos diferentes.", "Barbeiros ligados à equipa contam corretamente para a quota do plano."].map((text) => <div key={text} className="flex gap-3 border border-white/8 bg-black/20 p-4 text-sm leading-6 text-zinc-300"><Check className="mt-0.5 size-4 shrink-0 text-emerald-300" />{text}</div>)}</div><Link href="/plans" className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-white hover:text-emerald-200">Ver planos <ArrowRight className="size-4" /></Link></article></section>

      <section className="mx-auto max-w-7xl px-4 pb-18 sm:px-6 sm:pb-24 lg:px-8"><SectionHeading eyebrow="Planos" title="Começa simples e cresce sem trocar de sistema." text="O plano é definido pela barbearia e acompanha toda a equipa. Escolhe pela fase do negócio, não pela quantidade de logins." /><div className="mt-10 grid gap-3 lg:grid-cols-3">{plans.map((plan) => <article key={plan.name} className={`relative border p-5 sm:p-6 ${plan.recommended ? "border-emerald-400/30 bg-emerald-400/[0.055]" : "border-white/10 bg-zinc-900/35"}`}>{plan.recommended ? <span className="absolute right-4 top-4 border border-emerald-400/20 bg-emerald-400/[0.08] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-200">Mais escolhido</span> : null}<p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">{plan.subtitle}</p><h3 className="mt-3 text-xl font-semibold text-white">{plan.name}</h3><p className="mt-5 text-3xl font-semibold tracking-tight text-white">{plan.price}<span className="text-sm font-normal text-zinc-500">{plan.name === "Free" ? "" : " / mês"}</span></p><ul className="mt-6 space-y-3">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm text-zinc-300"><Check className="mt-0.5 size-4 shrink-0 text-emerald-300" />{feature}</li>)}</ul><Link href="/registo" className={`mt-7 inline-flex min-h-11 w-full items-center justify-center gap-2 text-sm font-semibold ${plan.recommended ? "bg-white text-zinc-950" : "border border-white/10 bg-white/[0.04] text-white"}`}>{plan.name === "Enterprise" ? "Falar com a equipa" : "Começar"}<ArrowRight className="size-4" /></Link></article>)}</div><p className="mt-5 text-xs text-zinc-600">Os limites e funcionalidades completos estão disponíveis em <Link href="/plans" className="text-zinc-400 underline-offset-4 hover:underline">/plans</Link>.</p></section>
