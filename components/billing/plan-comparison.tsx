...
export function PlanComparison() {
  return (
    <section className="mt-16" aria-labelledby="comparison-title">
      ...
      <div className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/60 shadow-[0_20px_80px_rgba(0,0,0,0.3)]">
              ...
              <tr className="border-b border-white/10 bg-white/[0.03]">
                ...
                {PLAN_ORDER.map((plan) => (
                  <th
                    key={plan}
                    scope="col"
                    className={`px-4 py-5 text-center text-sm font-semibold sm:px-6 ${
                      plan === PLANS.PRO ? "text-emerald-300" : "text-zinc-100"
                    }`}
                  >
                    {PLAN_NAMES[plan]}
                    {plan === PLANS.PRO && (
                      <span className="mt-1 block text-[10px] font-medium uppercase tracking-widest text-emerald-400/70">
                        Mais popular
                      </span>
                    )}
                  </th>
