...
import { ExternalLink, Loader2, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
export function SubscriptionManagementCard() {
  ...
  const formatDate = (value?: string | null) => value ? new Date(value).toLocaleDateString("pt-PT", { day: "2-digit", month: "long", year: "numeric" }) : "—";
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/80 p-6 shadow-[0_24px_90px_rgba(0,0,0,0.22)] sm:p-8">
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isAdministrativePlan && <span className="inline-flex items-center gap-1 rounded-full border border-sky-400/20 bg-sky-400/10 px-2.5 py-1 text-[11px] text-sky-200"><Sparkles className="size-3.5" /> Gerido pela Silentra</span>}
            {isTrial && <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-[11px] text-amber-200"><Zap className="size-3.5" /> Trial ativo</span>}
          </div>
          <h2 className="mt-4 text-2xl font-semibold tracking-tight text-white">Gestão da subscrição</h2>
        ...
        {hasStripeSubscription && !isAdministrativePlan ? (
          ...
          <a href="/plans" className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-5 text-sm font-semibold text-zinc-950 transition hover:bg-emerald-300">
            Ver planos
          </a>
        )}
      </div>
      ...
      <div className="relative mt-7 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/8 bg-black/20 p-4"><p className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Origem</p><p className="mt-2 text-sm font-medium text-zinc-100">{source}</p></div>
        <div className="rounded-2xl border border-white/8 bg-black/20 p-4"><p className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Estado</p><p className="mt-2 text-sm font-medium text-zinc-100">{isAdministrativePlan ? "Atribuído" : subscription?.cancel_at_period_end ? "Cancelamento agendado" : subscription?.status === "trialing" ? "Em trial" : hasStripeSubscription ? "Ativo" : "Gratuito"}</p></div>
