...
export function PricingCard({
  ...
  const handleAction = async () => {
    try {
      ...
      if (subscription?.status === "active") {
        if (onManagePortal) onManagePortal();
        else await upgrade();
        return;
      }
  ...
  const buttonConfig = useMemo(() => {
    ...
    return {
      label: tier === "free" ? "Usar Plano Gratuito" : "Começar com o Pro",
      disabled: false,
      variant: popular ? ("primary" as const) : ("outline" as const),
    };
  ...
  return (
    <div
      className={`relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border p-6 transition-[border-color,background-color,box-shadow,transform] duration-200 sm:p-7 ${
        isActivePaidPlan
          ? "border-emerald-500/60 bg-zinc-900/95 shadow-[0_20px_70px_rgba(16,185,129,0.15)] ring-1 ring-emerald-500/30"
          : popular
            ? "-translate-y-1 border-emerald-500/40 bg-zinc-900/95 shadow-[0_24px_80px_rgba(16,185,129,0.14)] ring-1 ring-emerald-500/10"
            : "border-white/10 bg-zinc-900/70 shadow-[0_18px_60px_rgba(0,0,0,0.25)] hover:-translate-y-0.5 hover:border-white/20"
      }`}
    >
      {popular && (
        <div className="absolute inset-x-0 top-0 h-0.5 bg-emerald-400" aria-hidden="true" />
      )}
      ...
      <div>
        ...
        <div className="mt-6 flex items-baseline gap-1">
          ...
          {tier !== "free" && <span className="text-xs text-zinc-500">/mês</span>}
        </div>
        {tier === "pro" && <p className="mt-2 text-xs font-medium text-emerald-300/90">Para crescer sem aumentar a complexidade.</p>}
        ...
        <div className="mt-7 space-y-3 border-t border-white/8 pt-6">
          <p className="text-xs font-medium uppercase tracking-[0.14em] text-zinc-500">Inclui</p>
          <ul className="space-y-3">
            {features.map((feature) => (
              <li key={feature} className="flex items-start gap-3">
                ...
                <span className="text-sm leading-5 text-zinc-300">{feature}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="mt-8 border-t border-white/8 pt-5">
        <button
          ...
          className={`inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-5 py-3 text-sm font-semibold transition-[background-color,border-color,box-shadow,transform] duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${
            buttonConfig.variant === "primary"
              ? "bg-emerald-400 text-zinc-950 shadow-[0_8px_24px_rgba(52,211,153,0.18)] hover:bg-emerald-300 hover:shadow-[0_10px_30px_rgba(52,211,153,0.24)]"
        ...
        >
          {loading ? <Loader2 className="size-4 animate-spin" /> : <><span>{buttonConfig.label}</span>{!buttonConfig.disabled && <ArrowRight className="size-4" />}</>}
        </button>
        {tier === "pro" && !isCurrentPlan && <p className="mt-2 text-center text-[11px] text-zinc-600">Cancela quando quiseres.</p>}
      </div>
    </div>
  );
}