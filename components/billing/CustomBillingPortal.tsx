...
export function CustomBillingPortal() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-3xl border border-white/10 bg-zinc-900 p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-400"><RefreshCw className="size-5" /></div><div><h3 className="font-semibold text-zinc-100">Escolher plano</h3><p className="text-sm text-zinc-500">Seleciona o plano e a modalidade de faturação.</p></div></div><div className="rounded-full border border-white/10 bg-zinc-950 p-1 text-xs"><button onClick={() => setBillingCycle("month")} className={`rounded-full px-3 py-1.5 ${billingCycle === "month" ? "bg-emerald-500 text-zinc-950" : "text-zinc-400"}`}>Mensal</button><button onClick={() => setBillingCycle("year")} className={`rounded-full px-3 py-1.5 ${billingCycle === "year" ? "bg-emerald-500 text-zinc-950" : "text-zinc-400"}`}>Anual</button></div></div>{billing.pricesLoading ? <Skeleton className="h-56 w-full bg-white/5" /> : <div className="grid gap-3 md:grid-cols-2">{(["pro", "enterprise"] as const).map((tier) => { const plan = billing.prices.find((item) => item.plan === tier && item.interval === billingCycle); if (!plan) return null; const current = hasActiveSubscription && subscription?.stripe_price_id === plan.id; const name = tier === "enterprise" ? "Barbers Enterprise" : "Barbers Pro"; return <div key={tier} className={`rounded-2xl border p-5 ${current ? "border-emerald-500/40 bg-emerald-500/5" : "border-white/10 bg-zinc-950/40"}`}><div className="flex justify-between gap-3"><div><h4 className="font-medium text-zinc-100">{name}</h4><p className="mt-1 text-xs text-zinc-500">{PLAN_DESCRIPTIONS[tier]} · Cobrança {billingCycle === "year" ? "anual" : "mensal"}</p></div><span className="shrink-0 font-semibold text-zinc-100">{formatPrice(plan)}<small className="font-normal text-zinc-500">/{billingCycle === "year" ? "ano" : "mês"}</small></span></div><ul className="mt-5 space-y-2 border-t border-white/10 pt-4">{PLAN_FEATURES[tier].map((feature) => <li key={feature} className="flex items-start gap-2 text-xs text-zinc-300"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-400" />{feature}</li>)}</ul><button disabled={current || billing.isChangingPlan} onClick={() => changePlan(plan.id)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-zinc-200 hover:bg-white/10 disabled:cursor-default disabled:opacity-50">{billing.isChangingPlan && <Loader2 className="size-3.5 animate-spin" />}{current ? <><Check className="size-3.5 text-emerald-400" />Plano atual</> : hasActiveSubscription ? "Mudar para este plano" : "Continuar para pagamento"}</button></div>; })}</div>}</section>
