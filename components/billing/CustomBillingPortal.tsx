...
import { AlertTriangle, Check, CreditCard, Loader2, RefreshCw, Trash2, WalletCards } from "lucide-react";
...
function planName(priceId: string | null | undefined, prices: BillingPrice[]) {
  return prices.find((plan) => plan.id === priceId)?.name ?? "Gratuito";
}

export function CustomBillingPortal() {
  const { subscription, loading: subscriptionLoading, cancel } = useSubscription();
  ...
  const cancelSubscription = async () => { try { await cancel(); toast.success("A subscrição será cancelada no fim do período atual."); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível cancelar a subscrição."); } };
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-zinc-900 to-zinc-900 p-6 sm:p-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><p className="text-xs font-semibold uppercase tracking-[.22em] text-emerald-400">Subscrição atual</p><h2 className="mt-3 text-3xl font-bold text-zinc-50">{currentPlan === PLANS.FREE ? PLAN_NAMES[PLANS.FREE] : planName(subscription?.stripe_price_id, billing.prices)}</h2><p className="mt-2 text-sm text-zinc-400">{subscription?.cancel_at_period_end ? "Cancelamento agendado" : hasActiveSubscription ? "Subscrição ativa" : "Plano gratuito"}</p></div><div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-zinc-300"><span className="block text-xs text-zinc-500">Próxima renovação</span><span className="mt-1 block font-medium text-zinc-100">{subscription?.current_period_end ? new Date(subscription.current_period_end).toLocaleDateString("pt-PT", { day: "2-digit", month: "long", year: "numeric" }) : "—"}</span></div></div></section>

    <section className="rounded-3xl border border-white/10 bg-zinc-900 p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-400"><CreditCard className="size-5" /></div><div><h3 className="font-semibold text-zinc-100">Método de pagamento</h3><p className="text-sm text-zinc-500">{primaryCard ? `${primaryCard.brand.toUpperCase()} •••• ${primaryCard.last4} · expira ${primaryCard.expMonth}/${primaryCard.expYear}` : "Sem cartão guardado"}</p></div></div><button onClick={createCardSetup} disabled={!subscription || billing.loading} className="rounded-full bg-emerald-500 px-4 py-2 text-xs font-semibold text-zinc-950 hover:bg-emerald-400 disabled:opacity-50">Adicionar cartão</button></div>{billing.paymentMethods.length > 0 && <div className="mt-5 space-y-2 border-t border-white/10 pt-4">{billing.paymentMethods.map((method) => <div key={method.id} className="flex items-center justify-between rounded-xl bg-white/[.03] px-3 py-2 text-xs text-zinc-300"><span>{method.brand.toUpperCase()} •••• {method.last4} {method.isDefault && <em className="ml-2 not-italic text-emerald-400">Predefinido</em>}</span><span className="flex items-center gap-3">{!method.isDefault && <button onClick={() => billing.setDefaultPaymentMethod(method.id)} className="text-emerald-400">Usar</button>}<button aria-label={`Remover cartão terminado em ${method.last4}`} onClick={() => removeCard(method.id)} className="rounded p-1 text-zinc-500 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="size-3.5" /></button></span></div>)}</div>}</section>

    <section className="rounded-3xl border border-white/10 bg-zinc-900 p-6"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-400"><RefreshCw className="size-5" /></div><div><h3 className="font-semibold text-zinc-100">Escolher plano</h3><p className="text-sm text-zinc-500">Começa grátis e faz upgrade quando precisares de mais ferramentas.</p></div></div>
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <div className={`rounded-2xl border p-5 ${currentPlan === PLANS.FREE ? "border-emerald-500/40 bg-emerald-500/5" : "border-white/10 bg-zinc-950/40"}`}><div className="flex justify-between gap-3"><div><h4 className="font-medium text-zinc-100">{PLAN_NAMES[PLANS.FREE]}</h4><p className="mt-1 text-xs text-zinc-500">{PLAN_DESCRIPTIONS.free}</p></div><span className="shrink-0 font-semibold text-zinc-100">0 €<small className="font-normal text-zinc-500">/mês</small></span></div><ul className="mt-5 space-y-2 border-t border-white/10 pt-4">{PLAN_FEATURES.free.slice(0, 6).map((feature) => <li key={feature} className="flex items-start gap-2 text-xs text-zinc-300"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-400" />{FEATURE_LABELS[feature]}</li>)}</ul><button disabled={currentPlan === PLANS.FREE || billing.isChangingPlan} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-zinc-200 disabled:cursor-default disabled:opacity-50">{currentPlan === PLANS.FREE ? <><Check className="size-3.5 text-emerald-400" />Plano atual</> : "Plano gratuito"}</button></div>
        {([PLANS.PRO, PLANS.ENTERPRISE] as const).map((tier) => { const plan = billing.prices.find((item) => item.plan === tier && item.interval === billingCycle); if (!plan) return null; const current = hasActiveSubscription && currentPlan === tier && subscription?.stripe_price_id === plan.id; const name = tier === PLANS.ENTERPRISE ? "Barbers Enterprise" : "Barbers Pro"; return <div key={tier} className={`rounded-2xl border p-5 ${current ? "border-emerald-500/40 bg-emerald-500/5" : "border-white/10 bg-zinc-950/40"}`}><div className="flex justify-between gap-3"><div><h4 className="font-medium text-zinc-100">{name}</h4><p className="mt-1 text-xs text-zinc-500">{PLAN_DESCRIPTIONS[tier]} · Cobrança {billingCycle === "year" ? "anual" : "mensal"}</p></div><span className="shrink-0 font-semibold text-zinc-100">{formatPrice(plan)}<small className="font-normal text-zinc-500">/{billingCycle === "year" ? "ano" : "mês"}</small></span></div><ul className="mt-5 space-y-2 border-t border-white/10 pt-4">{PLAN_FEATURES[tier].slice(0, 6).map((feature) => <li key={feature} className="flex items-start gap-2 text-xs text-zinc-300"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-400" />{FEATURE_LABELS[feature]}</li>)}</ul><button disabled={current || billing.isChangingPlan} onClick={() => changePlan(plan.id)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-zinc-200 hover:bg-white/10 disabled:cursor-default disabled:opacity-50">{billing.isChangingPlan && <Loader2 className="size-3.5 animate-spin" />}{current ? <><Check className="size-3.5 text-emerald-400" />Plano atual</> : hasActiveSubscription ? "Mudar para este plano" : "Continuar para pagamento"}</button></div>; })}
      </div>
