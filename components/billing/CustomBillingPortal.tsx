...
export function CustomBillingPortal() {
  const { subscription, loading: subscriptionLoading, cancel, resume } = useSubscription();
    ...
    <section className="rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-zinc-900 to-zinc-900 p-6 sm:p-8">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-bold text-zinc-50">{currentPlan === PLANS.FREE ? PLAN_NAMES[PLANS.FREE] : planName(subscription?.stripe_price_id, billing.prices)}</h2>
          <p className="mt-2 text-sm text-zinc-400">{subscription?.cancel_at_period_end ? "Cancelamento agendado" : hasActiveSubscription ? "Subscrição ativa" : "Plano gratuito"}</p>
        </div>
