...
export function EmptyState({
  ...
  const action = actionLabel ? (
    onAction ? (
      <Button type="button" onClick={onAction} className="min-h-11 rounded-lg bg-zinc-50 px-4 text-zinc-950 hover:bg-white">
        {actionLabel}
        <ArrowRight className="ml-2 size-4" aria-hidden="true" />
      </Button>
    ) : actionHref ? (
      <Button asChild className="min-h-11 rounded-lg bg-zinc-50 px-4 text-zinc-950 hover:bg-white">
  ...
  return (
    <div className="silentra-empty flex flex-col items-center justify-center px-5 py-12 text-center sm:py-14">
      <div className={`flex size-12 items-center justify-center rounded-xl border ${tones[tone]}`}>
