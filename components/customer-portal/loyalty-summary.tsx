  ...
  return (
    <section className="rounded-3xl border border-emerald-400/15 bg-gradient-to-br from-emerald-400/[0.08] via-white/[0.02] to-transparent p-5 shadow-xl sm:p-6">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300">Fidelização</p>
          <h2 className="mt-2 text-xl font-semibold text-white">{data.barbershop.name}</h2>
          <div className="mt-2 flex items-center gap-2 text-sm text-zinc-400">
            <Star className="size-4 fill-emerald-300 text-emerald-300" />
            <span>{points} pontos</span>
          </div>
        </div>
        <Link href={href} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white px-4 text-sm font-semibold text-zinc-950 transition hover:bg-zinc-200">
      ...
      {data.nextReward ? (
        <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="flex items-center justify-between gap-3 text-sm">
            <div className="flex items-center gap-2 text-zinc-300"><Sparkles className="size-4 text-emerald-300" /> Próxima recompensa</div>
            <span className="font-semibold text-white">{data.nextReward.points_cost} pts</span>
          </div>
          ...
          <p className="mt-2 text-xs text-zinc-500">Faltam {Math.max(0, data.nextReward.points_cost - points)} pontos.</p>
        </div>
      ) : (
