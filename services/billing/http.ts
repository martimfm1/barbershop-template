...
export function billingErrorResponse(error: unknown): NextResponse {
  ...
  console.error("billing.unexpected_error", { error: error instanceof Error ? error.name : "unknown" });
  return NextResponse.json({ error: "Unable to process billing request." }, { status: 500 });
}

export function appUrl(pathname: string): string {
  if (!APP_URL) throw new BillingError("Application URL is not configured.", "BILLING_NOT_CONFIGURED");
...
export function safeReturnUrl(value: unknown, fallbackPath: string): string {
  ...
  try {
    ...
    if (candidate.origin === new URL(fallback).origin) return candidate.toString();
  } catch {
...
export async function readJsonObject(request: Request): Promise<Record<string, unknown>> {
  ...
  throw new BillingError("Request body must be a JSON object.", "INVALID_PRICE");
}
