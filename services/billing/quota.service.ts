...
export function getRequiredPlanForResource(
...
export async function assertWithinPlanLimit(
  ...
  const plan =
    await SubscriptionService.getAccessPlanForBarbershop(barbershopId);
  ...
  if (current >= limit) {
    throw new QuotaError(
      ...
      limit,
      plan,
      getRequiredPlanForResource(resource, current + 1),
    );
  }
}
