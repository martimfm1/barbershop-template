...
export class BillingService {
  ...
  static async createCheckoutSession(input: CreateCheckoutInput): Promise<string> {
    if (!planForPrice(input.priceId)) {
      throw new BillingError("The requested price is not available.", "INVALID_PRICE", { priceId: input.priceId });
    }

    const existing = await SubscriptionService.getActiveForUser(input.userId);
    if (existing) {
      throw new BillingError("An active subscription already exists; use the billing portal.", "SUBSCRIPTION_NOT_ACTIVE", { userId: input.userId });
    }
    ...
    const session = await getStripeClient().checkout.sessions.create({
      ...
      subscription_data: { metadata: { user_id: input.userId }, trial_period_days: TRIAL_PERIOD_DAYS },
    ...
    const prices = await Promise.all(
      [...PRICE_ID_TO_PLAN.keys()].map(async (priceId) => {
        ...
        return {
          ...
          name: planForPrice(price.id) === PLANS.ENTERPRISE ? "Barbers Enterprise" : "Barbers Pro",
  ...
  private static async updateCancellation(userId: string, cancelAtPeriodEnd: boolean): Promise<void> {
    ...
    if (!subscription?.stripe_subscription_id) throw new BillingError("No active subscription was found.", "SUBSCRIPTION_NOT_FOUND", { userId });
  ...
  static async getCustomerId(userId: string): Promise<string> {
    ...
    const { error: writeError } = await createAdminClient().from("customers").upsert(
      { user_id: userId, stripe_customer_id: subscription.stripe_customer_id },
      { onConflict: "user_id" },
    );
  ...
  static async getPaymentMethods(userId: string) {
    ...
    try { customer = await this.getCustomerId(userId); }
      ...
      if (error instanceof BillingError && error.code === "CUSTOMER_NOT_FOUND") return [];
      throw error;
    }
    const stripe = getStripeClient();
  ...
  static async updatePaymentMethod(userId: string, action: "set_default" | "remove", paymentMethodId: string): Promise<void> {
    ...
    if (defaultId === paymentMethodId) {
      await stripe.customers.update(customer, { invoice_settings: { default_payment_method: null as unknown as string } });
    }
    await stripe.paymentMethods.detach(paymentMethodId);
  }

  static async updatePlan(userId: string, newPriceId: string): Promise<void> {
    if (!planForPrice(newPriceId)) throw new BillingError("The requested price is not available.", "INVALID_PRICE", { newPriceId });
    const subscription = await SubscriptionService.getActiveForUser(userId);
